...
// question id; engine/packs/interview.mjs), and the rule overrides / acceptances that
// exist BECAUSE this pack is declared (`rules`, `accept` — they may name any
// rule; the entry is their provenance). `via` is written by
// resolveDeclaredPacks on materialized dependencies (engine/packs/registry.mjs).
...
export function loadConfig(root) {
  ...
  for (const entry of Array.isArray(raw.packs) ? raw.packs : []) {
    ...
    if (entry.via !== undefined) {
      ...
      // (engine/packs/registry.mjs) — normalized through so readers (the adoption
