...
// The root that vendors wholesale into every consumer (mount/DESIGN.md): the
// whole engine lives under engine/ — the checks runner (engine/checks/), the
// mount machinery (engine/mount/), and the pack/skill machinery
// (engine/packs/, engine/skills/) — so "copy the engine" is one folder copy;
// tests live structurally apart in engine-tests/.
