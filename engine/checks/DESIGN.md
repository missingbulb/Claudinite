...
`engine/skills/registry.mjs` and run by the same engine. Skill checks are **never declared and always
...
`local_packs/<id>`, the bare id accepted while the fleet migrates — see checks/README.md). Both
...
- **Now (works with both mount methods):** skills live in `.claudinite/skills/<name>/`; each
  pack declares the skills it requires, and the SessionStart hook `engine/skills/mount-skills.mjs`
