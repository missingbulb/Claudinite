...
// This test lives at <repo>/engine/mount/vendor.test.mjs.
...
// REAL modules it imports (engine/pack_loader/registry.mjs, engine/checks_helpers/imports.mjs — all
...
function makeCanon({ packs = [], skills = [] } = {}) {
  ...
  mkdirSync(join(root, 'engine', 'checks_helpers'), { recursive: true });
  ...
  writeAt(root, 'engine/README.md', 'canon doc\n');
  writeAt(root, 'engine/test/runner.test.mjs', 'stub\n');
  writeAt(root, 'engine/hooks/session-start.sh', 'stub\n');
  writeAt(root, 'engine/mount/DESIGN.md', 'canon doc\n');
  // machinery roots: top-level .mjs picked up, tests and dirs' docs not
  writeAt(root, 'engine/pack_loader/load-active-prose.mjs', 'stub\n');
  writeAt(root, 'engine/pack_loader/env.mjs', 'stub\n');
...
test('structural set: engine roots + machinery + declared pack + its skills, exact; tests, engine docs, preferences all out', async () => {
  ...
  const expected = [
    ...
    'engine/hooks/session-start.sh',
    'engine/mount/vendor.mjs',
    'engine/pack_loader/env.mjs',
    'engine/pack_loader/load-active-prose.mjs',
    'engine/pack_loader/registry.mjs',
