...
# (engine/packs/env.mjs, driven by the repo's .claudinite-checks.json), NOT here — so
# this script never changes as requirements evolve.
#
# Vendored-mount only (mount/DESIGN.md): the corpus is tracked, already in the
...
node .claudinite/shared/engine/packs/env.mjs install
