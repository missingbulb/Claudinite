...
# This script lives in <corpus>/engine/mount/. The preferences step lives
# beside it (per-user content is never vendored, so its step is mount
# machinery — see DESIGN.md); the remaining step scripts are engine machinery
# one level up (<corpus>/engine/packs, <corpus>/engine/skills).
engine="$(dirname "$here")"
...
run_step load-active-prose  node "$engine/packs/load-active-prose.mjs"
run_step mount-skills       node "$engine/skills/mount-skills.mjs"
run_step env-check          node "$engine/packs/env.mjs" check
run_step interview-check    node "$engine/packs/interview.mjs" check
