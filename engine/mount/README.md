...
| [`inject-preferences.sh`](../hooks/inject-preferences.sh) | The **preferences step**: injects the current user's `preferences/<email>.md` — local copy first, single-file HTTPS fetch otherwise (preferences are per-user and never vendored). Fail-soft: any miss is a one-line note, never a halt. |
...
- `engine/pack_loader/load-active-prose.mjs` — emits the active packs' prose.
- `engine/skill_loader/mount-skills.mjs` — (re)generates the skill mounts.
- `engine/pack_loader/env.mjs check` — the cloud env-requirement assertion.

The enforcement hooks stay in `checks/` (`stop-hook.mjs`, `pretooluse-guard.mjs`) and the shared
logger in `engine/checks_helpers/hooklog.mjs`. `mount/` is the *loading* story; those are separate concerns.
