...
// This test lives at <repo>/engine/mount/apply-vendor.test.mjs.
...
function makeCanon() {
  ...
  for (const f of ['apply-vendor.mjs', 'vendor.mjs']) {
    copyFileSync(join(MOUNT_DIR, f), join(root, 'engine', 'mount', f));
  }
...
test('convergence is whole-set: stale files vanish, drift reverts, everything outside shared/ is untouched', async () => {
  ...
  writeAt(target, '.claudinite/shared/engine/check_the_world.mjs', 'locally edited\n');
  ...
  assert.equal(readFileSync(join(target, '.claudinite', 'shared', 'engine', 'check_the_world.mjs'), 'utf8'), 'engine v2\n');
...
function gitify(canon) {
  ...
  writeFileSync(join(canon, 'engine', 'check_the_world.mjs'), 'engine v3\n');
  g('add', '-A');
  g('commit', '-q', '-m', 'c2');
...
test('#328: a canon tree nested in a FOREIGN git repo is rootless — upward .git discovery must not speak for the canon', async () => {
  ...
  mkdirSync(join(canon, 'engine', 'checks_helpers'), { recursive: true });
