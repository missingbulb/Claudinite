...
Version 60822.1's was dispatched against `1f13e57` — this change's tree, with only
the row you are reading edited after it. An earlier rehearsal
([run 32604851895](https://github.com/missingbulb/Claudinite/actions/runs/32604851895))
qualified the first half of the same change and is superseded by this one, which
qualifies the whole: the canary converged both flows against a canon whose packs
declare no `id` and carry their rules in directories, and its own mount then loaded
them and passed its self-test.
