...
| 60823.1 | 2026-08-23 | [run 32609851921](https://github.com/missingbulb/Claudinite/actions/runs/32609851921) | The member declaration is renamed `.claudinite-settings.json` and reshaped (#1252): each pack's installed version moves onto that pack's own entry, `engineVersion` bubbles to the top level, the `claudinite` and `maintenance` blocks are retired, and `taskScheduler.endpoints` becomes `agenticTaskInvocationEndpoints`. The delivery preference becomes the `dailyClaudiniteUpdatesRequirePrReview` override. Every reader takes both settings-file names until no member carries the old one. |
| 60902.1 | 2026-09-02 | [run 33684042486](https://github.com/missingbulb/Claudinite/actions/runs/33684042486) | The engine snapshot accumulated since 60823.1, qualified as one release (#1630): a pack's `references.md` stays canon-side and never vendors (#1618), the settings validator accepts `disabledTasks` (#1593), an unplaceable declared-check key is reported rather than thrown on (#1544), the executor carries repo variables as one `toJSON(vars)` bag (#1494), the withhold lane re-opens for workflow files (#1511), a push onto the base branch is judged instead of skipped (#1523), and auto-merge policies gain inline folder scoping (`under:<dir>`) and intersection (`&&`) (#1474). No engine migration ranges against it; the number is what says this tree has been rehearsed. |
| 60925.1 | 2026-09-25 | [run 36174030882](https://github.com/missingbulb/Claudinite/actions/runs/36174030882) | A check says what it does when it fails as `on_fail`, `block` or `advise`, and a settings override takes `off`, `advise` or `block` (#2318). The retired `severity` spelling (`blocking` or `advisory`) is still read, and mirrored on every finding, until #2319; the `on-fail-rename` record respells a member's own settings and local packs, and every pack version is gated behind this release, since an older engine throws on a declared check with no `severity`. |

Version 60925.1's was dispatched against `0116e35c`, this change's tree after its last rebase, with only the row
you are reading edited after it. What it qualifies is the rename's first update: the canary's
own files held the old spelling when the run began, and both flows' converged trees passed
their self-tests.

Version 60902.1's was dispatched against `dff0643` — this change's tree, with only
the row you are reading edited after it. Nothing in the snapshot changes a member's
own files, so what the run qualifies is the ordinary case: the canary converged both
flows against this ref and its mount passed its self-test.

Version 60823.1's was dispatched against `3455f72` — this change's tree, with only the
run id in the row you are reading edited after it. Earlier runs qualified the same
change against the trees it had before two rebases; this one is the tree that actually
ships. What it qualifies specifically is the settings-file rename: the canary held a
declaration spelled the old way when the run began, and the converged tree passed its
self-test anyway, which is the shape every member is in for the window between this
landing and its own converge.
