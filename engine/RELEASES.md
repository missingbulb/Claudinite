...
| 60822.1 | 2026-08-22 | [run 32604851895](https://github.com/missingbulb/Claudinite/actions/runs/32604851895) | A pack manifest stops restating its own directory (#1246): `id`, `prose`, `badge` and `skills` are read off the pack tree, with a `pack.mjs` declaration overriding only where a pack genuinely differs. Every pack version is gated behind this release — a manifest with no `id` is dropped outright by an older loader, which fails the mount self-test and stops that member converging at all. |
...
Version 60822.1's was dispatched against `6473cef` — this change's tree. Two edits
landed after it: the row you are reading, and a rehearsal fixture shape, which is
canon-internal test material no mount carries. It is what qualifies the pack-manifest
convention specifically: the canary converged both flows against a canon whose packs
declare no `id`, and its own mount then loaded them and passed its self-test.
