...
| 60925.1 | 2026-09-25 | [run 36165536547](https://github.com/missingbulb/Claudinite/actions/runs/36165536547) | A check says what it does when it fails as `on_fail`, `block` or `advise`, and a settings override takes `off`, `advise` or `block` (#2318). The retired `severity` spelling (`blocking` or `advisory`) is still read, and mirrored on every finding, until #2319; the `on-fail-rename` record respells a member's own settings and local packs, and every pack version is gated behind this release, since an older engine throws on a declared check with no `severity`. |

Version 60925.1's was dispatched against `ae7834ab`, this change's tree, with only the row
