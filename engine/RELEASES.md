...
| 60823.1 | 2026-08-23 | [run 32609288065](https://github.com/missingbulb/Claudinite/actions/runs/32609288065) | The member declaration is renamed `.claudinite-settings.json` and reshaped (#1252): each pack's installed version moves onto that pack's own entry, `engineVersion` bubbles to the top level, the `claudinite` and `maintenance` blocks are retired, and `taskScheduler.endpoints` becomes `agenticTaskInvocationEndpoints`. The delivery preference becomes the `dailyClaudiniteUpdatesRequirePrReview` override. Every reader takes both settings-file names until no member carries the old one. |

Version 60823.1's was dispatched against `d4c8cd7` — this change's tree rebased onto
60822.1, with only the run id in the row you are reading edited after it. An earlier
run qualified the same change against its pre-rebase tree; this one is the tree that
actually ships. What it qualifies specifically is the
settings-file rename: the canary held a declaration spelled the old way when the run
began, and the converged tree passed its self-test anyway, which is the shape every
member is in for the window between this landing and its own converge.
