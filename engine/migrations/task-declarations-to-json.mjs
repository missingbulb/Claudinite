...
//
// The module's comments do not survive: JSON carries none, and the ones these
// files held mostly restated the assignment beside them. What was rationale
// belongs in the task's README or the pack's — the conversion prints each
// dropped comment block so the person running it can move what is worth moving.
// A field that is not data (the retired `precondition()` function) is dropped
// too, and named in the report line: the contract rejects it either way.
