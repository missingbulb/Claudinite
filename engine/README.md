...
```sh
node engine/check_the_world.mjs             # whole-repo sweep (the default — Stop hook and CI both run this)
node engine/check_the_world.mjs --changed   # transitional: only files changed vs the merge-base with main
node engine/check_the_world.mjs --list      # machine-readable rule catalog (id, severity, description, doc)
node engine/check_the_world.mjs --init      # write .claudinite-checks.json — the baseline plus the fingerprinted packs
...
- **packs** — the declared packs; the closed set that executes. **No pack runs undeclared** —
  ...
  engine resolves both forms to the bare id ([`packEntryId`](pack_loader/registry.mjs)), so a bare local
...
- **No CI job in the standard wiring** — edits made outside Claude sessions surface at the next
  ...
  `node engine/check_the_world.mjs` itself; same sweep, same messages.
...
`detect`, `rules`, and optional `prose`) — [engine/pack_loader/registry.mjs](pack_loader/registry.mjs)
discovers it structurally, no list to edit.

**Shared helpers carry mechanism, not policy.** A `engine/checks_helpers/` helper owns only the walking —
resolve a file set, find the lines a pattern matches, list the change's added lines
([checks_helpers/lines.mjs](checks_helpers/lines.mjs)) — never one rule's forbidden tokens, file filters, or failure
...
[`loadConfig`](checks_helpers/context.mjs) reports malformed JSON and an unknown top-level property; the runner adds
...
[`resolveDeclaredPacks`](pack_loader/registry.mjs) pulls each declared pack's `requires` closure into the
