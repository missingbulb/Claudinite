...
| [`vendor.mjs`](compute-vendor-set.mjs) | The **vendor-set computation** for the incoming tracked mount ([DESIGN.md](DESIGN.md)): a repo's pack declaration → the minimal corpus file set that repo persists under `.claudinite/shared/`. Not yet wired into the live mount. |
| [`inject-preferences.sh`](../hooks/steps/inject-preferences.sh) | The **preferences step**: injects the current user's `preferences/<email>.md` — local copy first, single-file HTTPS fetch otherwise (preferences are per-user and never vendored). Fail-soft: any miss is a one-line note, never a halt. |
...
- `engine/skill_loader/mount-skills.mjs` — (re)generates the skill mounts.
