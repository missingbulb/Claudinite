...
try {
  // This hook is engine machinery (<corpus>/engine/skills/); the canon skills
  // it mounts live in the content tree at <corpus>/skills.
  const corpusRoot = dirname(dirname(dirname(fileURLToPath(import.meta.url))));
  const skillsDir = join(corpusRoot, 'skills');
  ...
  const { loadPacks, isActive } = await import(join(corpusRoot, 'engine', 'packs', 'registry.mjs'));
