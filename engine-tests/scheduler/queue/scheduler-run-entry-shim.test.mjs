...
test('the retired `tick.mjs` name still starts a scheduler run', async () => {
  const shim = read('engine/scheduler/queue/tick.mjs');
  ...
  const mod = await import('../../../engine/scheduler/queue/scheduler-run.mjs');
...
test('the shim says which name was used, so a stale workflow is visible in its own log', () => {
  assert.match(read('engine/scheduler/queue/tick.mjs'), /console\.log\(/);
});
...
test('the workflow-shape check accepts either entry point', () => {
  ...
  const rule = JSON.stringify(declared).match(/engine\\+\/scheduler\\+\/queue\\+\/\(([^)]+)\)/);
...
test('canon runs the current entry point in both its workflow and the stub it ships', () => {
  for (const wf of ['.github/workflows/claudinite-scheduler.yml', 'engine/scheduler/stubs/claudinite-scheduler.yml']) {
