// The standing-tracker helpers a task's own code_work calls. What these pin is the
...
// exists" — plus the re-entrancy every code_work helper owes, since a reclaimed
// claim runs code_work again over its own half-done work.
...
test('find-or-create twice creates exactly one tracker', async () => {
  // Re-entrancy, not idempotence by luck: a reclaimed claim re-runs code_work, and a
