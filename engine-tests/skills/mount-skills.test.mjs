...
import { makeRepo, cleanup, git } from '../checks/helpers.mjs';
...
function makeCorpus({ packs, skills }) {
  ...
  copyFileSync(join(REPO_ROOT, 'engine', 'skills', 'mount-skills.mjs'), join(root, 'engine', 'skills', 'mount-skills.mjs'));
...
function mount(corpus, project) {
  const r = spawnSync('node', [join(corpus, 'engine', 'skills', 'mount-skills.mjs')], {
    encoding: 'utf8',
...
test('mount-skills: the real corpus mounts every basics skill into a consumer', () => {
  const project = makeRepo({ changed: { '.claudinite-checks.json': '{ "packs": ["basics"] }\n' } });
  try {
    const r = spawnSync('node', [join(REPO_ROOT, 'engine', 'skills', 'mount-skills.mjs')], {
      encoding: 'utf8',
      env: { ...process.env, CLAUDE_PROJECT_DIR: project },
    });
    assert.equal(r.status, 0, r.stderr);
