...
test('the engine half is the engine tree plus the catalog nothing else owns', () => {
  ...
  assert.equal(isEngineFile('packs/acme-pack/RULES.md'), false);
  assert.equal(isEngineFile('packs/acme-pack-c/migrations/2026-08-11-x/migration.mjs'), false, 'a pack\'s records are the pack flow\'s');
});
