...
- [tasks/agentic-documentation.md](tasks/agentic-documentation.md) — **Read before writing or editing an instruction doc for Claude** (a practice doc, a project `CLAUDE.md`, a routine spec). Right altitude (concrete but not brittle), the attention-budget cost model (terse → better adherence), the anatomy of one instruction (trigger + imperative + why-only-if-it-matters), examples, and doc structure. Grounded in Anthropic's published guidance.
