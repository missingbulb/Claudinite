...
- [tasks/agentic-documentation.md](tasks/agentic-documentation.md) — **Read before writing or editing an instruction doc for Claude** (a practice doc, a project `CLAUDE.md`, a routine spec). The context-budget cost model, the anatomy of one instruction (trigger + imperative + why-only-if-it-matters), length/detail, and doc structure.
