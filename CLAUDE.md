...
Unlike everything below, these rules aren't tied to one kind of task, so the index **`@`-imports** them — the one deliberate exception to the soft-pointer rule above — to force-load them every session. They keep their own topic files under `general/` (single source of truth, and a home for the lessons routine to add to); they're simply not soft-gated. Read them as part of this index:

@general/working-discipline.md
@general/task-lifecycle.md
