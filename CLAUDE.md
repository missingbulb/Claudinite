...
2. **Who you're working with → [preferences/](preferences/).** One file per person who uses Claudinite, **named for that person's email address** (e.g. `preferences/jane@example.com.md`), holding their interaction preferences and trigger phrases. **This is a crucial, imperative read — an exception to the soft-pointer rule above: at session start, before any work, read `preferences/$CLAUDE_CODE_USER_EMAIL.md`. Skipping it is all but certain to cause problems for the user, because you'll miss the trigger phrases and conventions they rely on** — the file whose name is exactly the current user's email, taken verbatim from the `CLAUDE_CODE_USER_EMAIL` environment variable (`@` and `.` are valid in filenames; no escaping). Read only that file. If no file with that exact name exists, there are no personal preferences to apply — proceed on the general rules alone.
...
- [general/working-discipline.md](general/working-discipline.md) — general working habits (problem-first: agree on the problem before implementing a requested change, confirm-before-building, the warnings policy).
...
**At session start, it is imperative that you read `preferences/$CLAUDE_CODE_USER_EMAIL.md`** — the single file whose name is exactly the current user's email (from the `CLAUDE_CODE_USER_EMAIL` environment variable, used verbatim — `@` and `.` need no escaping). This is crucial: failing to read it is all but definite to cause problems for the user, since you'll act without the preferences and trigger phrases they depend on. Read only that file; other users' preferences don't apply. If no file with that exact name exists, there are none to apply.
