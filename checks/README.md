...
node checks/run.mjs --init      # write .claudinite-checks.json — universal plus the fingerprinted packs
...
```json
{
  "packs": ["universal", "github-actions"],
...
- **packs** — the declared packs; the closed set that executes. **No pack runs undeclared** —
  the `universal` baseline too is declared explicitly (`--init` seeds it; the nightly
