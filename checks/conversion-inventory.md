...
| **chrome-extension-release.md — the contract** | **check** `chrome-extension-release/` conformance suite: three workflow stubs with exact `name:`s · no surviving `__TOKENS__` · manifest ↔ `package.json` version sync · kebab-cased zip name · README Install/Releasing sections · `PRIVACY.md` present · every manifest permission disclosed in `PRIVACY.md` (test-the-world) · an *added* permission raises an advisory to open the store-dashboard justification issue (test-the-work) · release machinery under `dev/build/release/` · store-asset inventory (128px icon, ≥1 screenshot) · assets generator-backed · dependency-free bumper/filter scripts *(adv)*. Listing copy + permission justifications are Chrome Web Store dashboard state (no repo file); first publication is a one-time tracking issue, not a `STORE-LISTING.md`; no per-repo `releasing.md`. Doc keeps the setup narrative and manual store steps |
