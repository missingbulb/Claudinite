...
3. The **re-bootstrap** — independent of the growth phases (no ordering constraint), run per member repo **only** (never the home repo — the canon doesn't mount itself). It refreshes each consumer's Claudinite mount and *wiring* by re-running the idempotent bootstrap ([../bootstrap.md](../bootstrap.md), the content of the `adopt-claudinite` skill). This is the only step that renews the parts that **don't** self-update at session start — the `sync-claudinite.sh` generated artifact, the `settings.json` hook registrations, the `.claude/skills/` symlinks, the gitignore rules, the pack declaration (Method B re-syncs the corpus *contents* each session, but never this wiring; Method A's pinned submodule drifts in both). It commits any refreshed wiring **directly to the member's default branch** — no PR, since this is mechanical, idempotent regeneration the owner has opted not to gate — and, since these repos are already on the routine's maintenance access list (that's how it reached them), it **skips opening bootstrap's enrollment issue** (Part 4) — the repo is provably already enrolled. That same proof also **closes** the enrollment issue: reaching the repo means the access-list ask is fulfilled, so if the enrollment issue (title `Enroll <PROJECT_NAME> in Claudinite fleet maintenance`, found **by title**) is still open, close it (`completed`) with a one-line comment noting maintenance now reaches the repo. Idempotent — most days it's already closed and there's nothing to do. Most days nothing has drifted and it commits nothing; the owner's "try to re-bootstrap" is exactly that — attempt it every day, act only when there's drift.
...
- **Never re-bootstrap the home repo** — re-bootstrap targets consumers only (the canon doesn't mount itself); only the repo tidy-up runs against home. And never *re-open* the enrollment issue on a re-bootstrap — a repo the routine reached is already enrolled, so re-bootstrap closes a still-open one rather than opening another.
