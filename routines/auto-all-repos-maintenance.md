...
- **Growth** ([../growth/README.md](../growth/README.md)) — `growth-extract-new-instructions` (`order: growth:1`), `growth-dedup-local-instructions` (`growth:3`); `growth-promote-to-claudinite` runs once, central, **post-barrier** — not a planned unit, since its input is *this night's* extractions. `growth-stack-manifest` (`order: null`, weekly) is an **independent** growth unit — outside the phased barrier — that converges a project's "Stack manifest" issue (stage 1 of pack discovery).
