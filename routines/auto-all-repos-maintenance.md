...
> Run the fleet daily maintenance routine exactly as specified in `<path/to/routines/auto-all-repos-maintenance.md>`. **Run the planner**: run the core planner (`routines/fleet/plan.mjs`) over the repos you can reach — it enumerates the accessible repos and decides every unit in code, **pack-agnostically (it never dispatches an enforcer pack's coverage workflow, and a broken enforcer pack is one isolated unit, not a missing plan)**; do not decide the worklist by your own reasoning. **Then dispatch each unit** as an isolated **plain subagent (never the Workflow orchestration tool, whose interactive opt-in an unattended routine can't answer)** at the tier its `smarts` names — or, for `smarts: none`, as a direct code/tool action, no subagent — running its `worker` doc against its repo with the unit's `targets`. Honor the ordering: independent units (tidy, baselining, pack tasks, `growth-discover-packs`) concurrently; the growth units phased with a barrier between each — all `growth:1`, then `growth-promote-to-claudinite` once (only if an extract produced output), then all `growth:3`. Run the tidy tasks against this home repo too, and work the sweep's adoption queue. Await any dispatched Action to completion, not to its trigger. Wait for everything to settle. You are a sequencer only: never decide the worklist by your own reasoning, never run a worker's logic, commit docs, edit the canon, or write to any step's tracking issue. Log **only failures** — the planner code failing, a plan that couldn't cover the reachable fleet, a unit that didn't complete, or a barrier never reached — to this routine's own standing tracking issue in this home repo (found **by title**), naming the repo and task affected; on a clean day, log nothing.
