...
3. The **[fleet bootstrap sweep](auto-fleet-bootstrap.md)** — independent of the growth phases (no ordering constraint), never run against the home repo. It dispatches its fleet-credentialed **census executor** (a `workflow_dispatch`-only GitHub Action — deliberately no schedule of its own), **re-bootstraps and aligns** every member with its declared packs' current checks, and **adopts** every census-queued repo it can reach — delivering member-side changes per each member's explicit `maintenance.delivery` flag. The census contract, alignment bounds, delivery semantics, tracking, and never-list live in its doc.
...
The result is the day's **member-repo list**. The [fleet bootstrap sweep](auto-fleet-bootstrap.md) re-bootstraps these members; its adoption targets come from its own census executor, not from this enumeration. If enumeration itself fails or returns only partially, you **cannot guarantee the fleet was covered** — treat that as a failure and log it (see Tracking).
