...
3. The **[fleet bootstrap sweep](auto-fleet-bootstrap.md)** — independent of the growth phases (no ordering constraint), never run against the home repo (the canon doesn't mount itself). One idempotent bootstrap ([../bootstrap.md](../bootstrap.md)) across two populations: every **member** gets a **re-bootstrap** — refreshing the mount wiring that doesn't self-update at session start, committing drift directly to its default branch, closing a lingering enrollment issue — and every **uncovered** repo under the owner's account (no opt-in signal; not a fork, archived, or on the owner's [opt-out list](fleet-bootstrap-opt-out.md)) gets **adopted** via the same bootstrap, with bootstrap Part 4's enrollment issue opened — so the account converges to covered-or-opted-out and yesterday's new repo is tomorrow's maintained member. The re-bootstrap's refresh scope, the uncovered test, the opt-out semantics, its tracking, and its never-list all live in its doc.

**Extending it:** a new per-repo growth phase is a line in the growth lifecycle; a new independent routine (like the repo tidy-up or the bootstrap sweep) is another entry here. Never register a new schedule and never add a per-repo dispatch layer — the single scheduled routine runs everything in every opted-in repo automatically the next day.
...
- **Method B (tarball sync):** a version-controlled **`.claudinite/sync-claudinite.sh`** hook — or, on a not-yet-migrated repo, the legacy **`.claudinite/.gitkeep`** marker (the re-bootstrap migrates it to the hook layout).
...
The result is the day's **member-repo list**. Keep the **full enumeration** too — both lists feed the [fleet bootstrap sweep](auto-fleet-bootstrap.md): it re-bootstraps the members, and adopts from the enumeration's complement (the owner's repos carrying **no** signal, minus its own exclusions, per its doc). If enumeration itself fails or returns only partially, you **cannot guarantee the fleet was covered** — treat that as a failure and log it (see Tracking).
...
Dispatch **each per-repo run as its own subagent** — **plain subagent dispatch (the Agent/Task tool), never the multi-agent-orchestration Workflow tool.** The Workflow tool mandates an interactive human opt-in on **every** run (its prompt only disappears under bypass-permissions/headless modes you cannot rely on the scheduler being in), so a routine that fans out through it stalls on step one waiting for an "Allow" no one is there to click — the exact failure this routine exists to prevent. Plain subagent dispatch carries no such gate, and the orchestrator honors the barriers itself (launch a phase's runs, wait for all to settle, then start the next). Instruct each subagent to run that phase per its doc (targeting the repo and its default branch). The subagent boundary delivers the guarantee the owner cares about: **failure isolation** (a run that errors, stalls, or exits early fails *its own* subagent only), **context isolation** (a clean context per run), and **behavior unchanged** (each subagent runs its phase **exactly as that phase's doc specifies**). This routine adds **no** behavior to any phase.

Run the growth phases in their numbered order (phase 1 → 2 → 3 above), waiting at each barrier before the next. Run the independent **repo tidy-up** and **[fleet bootstrap sweep](auto-fleet-bootstrap.md)** concurrently alongside them — neither has an ordering dependency. Run the repo tidy-up per member repo **and** against the home repo; run the bootstrap sweep one subagent per target repo — every member (re-bootstrap) plus every uncovered repo it identifies (adoption), never the home repo — each run exactly per its doc. The sweep's subagents work over the API — never a clone — and commit directly to the target's default branch, safely concurrent with the growth phases (they touch different files: growth writes a project's local docs, the sweep only the mount wiring its doc names).
...
> Run the fleet daily maintenance routine exactly as specified in `<path/to/routines/auto-all-repos-maintenance.md>`. Enumerate **every** repo the GitHub token can access (page through all of them), keep only those carrying the tracked `.claudinite/` opt-in signal on their default branch (a `.claudinite/sync-claudinite.sh` hook or legacy `.claudinite/.gitkeep` file, or a `.claudinite` submodule in `.gitmodules` pointing at Claudinite), and skip archived/read-only repos. Then run the **growth lifecycle** (see `growth/README.md`) across them in isolated **plain subagents (never the Workflow orchestration tool, whose interactive opt-in an unattended routine can't answer)** — its phases in order, waiting for each to settle at the barrier before the next, each phase run per its doc (targeting the repo and its default branch; **extract commits directly to `main`**, while **promote and dedup open a PR** for the owner to approve — never merging). Run the independent **repo tidy-up** (`auto-repo-tidy.md`) concurrently, in every member repo **and** against this home repo. Also run, concurrently and independently, the **fleet bootstrap sweep** (`auto-fleet-bootstrap.md`), one isolated subagent per target repo, exactly per that doc: **re-bootstrap** every member repo (never this home repo) — re-run the idempotent bootstrap (`bootstrap.md`) over the API to refresh its mount and wiring, committing directly to its default branch only if something drifted, skipping the enrollment issue and closing a still-open one (title `Enroll <PROJECT_NAME> in Claudinite fleet maintenance`, found by title) with a one-line comment — and **adopt** every uncovered repo — each repo under this account with **no** opt-in signal (per a marker check that must *succeed*: a denied check means unknown, not uncovered — log it as pending-grant, don't adopt) that is not a fork, not archived, and not named in the opt-out list (`routines/fleet-bootstrap-opt-out.md`, read fresh from this home repo's default branch; if it can't be read, adopt nothing and log the failure), running the same bootstrap (Method B), committing directly to that repo's default branch, opening the enrollment issue per bootstrap Part 4, and logging each adoption or failure to the sweep's standing tracker in this home repo (found by title `Claudinite fleet coverage`); on a day with no drift, nothing uncovered, and an unchanged picture, the sweep writes nothing. Wait for everything to settle. You are a sequencer only: never commit docs, edit the canon, or write to any step's tracking issue. Log **only failures** — a run that didn't complete, a barrier never reached, or a fleet you couldn't fully enumerate — to this routine's own standing tracking issue in this home repo (found **by title**), naming the repo and phase affected; on a clean day, log nothing.
...
- **Never maintain a non-opted-in repo** — growth, tidy-up, and the sweep's re-bootstrap half run only on repos with a tracked `.claudinite/` marker. The sole step that touches a non-member is the [fleet bootstrap sweep](auto-fleet-bootstrap.md)'s adoption half, and only to adopt it into the fleet — within the owner's account, never a fork or archived repo, and never one on the [opt-out list](fleet-bootstrap-opt-out.md).
- **Never run the bootstrap sweep against the home repo** — it targets consumers only (the canon doesn't mount itself); only the repo tidy-up runs against home. The enrollment-issue rules (open on adoption; skip-and-close on re-bootstrap) live in the sweep's doc.
- **Never fan out through the Workflow tool** — dispatch runs as plain subagents (Agent/Task). The Workflow tool's mandatory interactive opt-in has no one to answer it in an unattended routine, so it stalls the whole run on step one.
...
- **Never break the phase ordering** — run the growth phases in the lifecycle's order, waiting at each barrier; only the repo tidy-up and the bootstrap sweep are unordered.
