...
- **Pack discovery** ([../growth/discover-packs.md](../growth/discover-packs.md)) — `growth-discover-packs` runs once, central, **weekly** (on the full sweep) — like promote it's a central step, not a planned unit. A single process end to end: manifest each repo's stack, suggest a pack for each unhomed technology, populate it with distilled rules/checks, and open one PR per authored pack. Outside the phased growth barrier.
