...
- **Method B (tarball sync):** a version-controlled **`.claudinite/sync-claudinite.sh`** hook — or, on a not-yet-migrated repo, the legacy **`.claudinite/.gitkeep`** marker (baselining migrates it to the hook layout).
...
- **Never maintain a non-opted-in repo** — growth, tidy-up, and the sweep's baselining half run only on repos with a tracked `.claudinite/` marker. The sole step that touches a non-member is the [fleet bootstrap sweep](auto-fleet-bootstrap.md)'s adoption half, and only to adopt it into the fleet — within the owner's account, never a fork or archived repo, and never one on the [opt-out list](fleet-bootstrap-opt-out.md).
- **Never run the bootstrap sweep against the home repo** — it targets consumers only (the canon doesn't mount itself); only the repo tidy-up runs against home. The enrollment-issue rules (open on adoption; skip-and-close on baselining) live in the sweep's doc.
