# Fleet-coverage opt-out list

The repos the [fleet-coverage sweep](auto-fleet-coverage.md) must **never adopt**. The sweep
...
mount (see the sweep's never-list).

The sweep reads **only the entries under the "Opted out" heading below**, fresh from this file on
the home repo's default branch at the start of every run. One bullet per repo — the full
