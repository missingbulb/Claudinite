...
import packGapScan from '../../packs/grow_with_claudinite/run_daily/growth-pack-gap-scan.mjs';
...
test('growth-pack-gap-scan (grow_with_claudinite): weekly-only, independent of the growth barrier', async () => {
  assert.equal(packGapScan.full_sweep_supported, true);
  assert.equal(packGapScan.order, null); // outside the growth:1 → promote → growth:3 barrier
  // Slow-moving signal: fires only on the weekly full sweep, not on day-to-day change.
  assert.equal((await packGapScan.gate(REPO, S())).run, false);
  assert.equal((await packGapScan.gate(REPO, S({ projectChanged: true }))).run, false);
  assert.equal((await packGapScan.gate(REPO, S({ canonChanged: true }))).run, false);
  assert.equal((await packGapScan.gate(REPO, S({ fullSweep: true }))).run, true);
});
