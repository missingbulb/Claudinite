...
- **A member's own local packs contribute tasks the same way — planned by default.** A repo's
  `.claudinite/local_packs/` packs carry `run_daily` descriptors exactly like a canon pack, but they
  live in the *member* tree, not the canon checkout the planner reads. The planner's `localTasksFor`
  seam defaults to the real reader ([local-tasks.mjs](local-tasks.mjs)): it fetches each member's
  local-pack descriptors over the same MCP `gh`, imports each self-contained descriptor, and tags it
  with its pack and `workerRepo` so the unit's worker is read from the member. A local task is gated
  by the member's declaration exactly like a canon task (a descriptor shadowing a canon pack id is
  never planned — canon wins), and the canon home's own curation tasks ride this path nightly.
  Running the member's own gate code centrally is safe under the single-owner fleet model — the same
  trust the routine already extends to every member-authored worker. The seam stays injectable for
  tests, and `localTasksFor: null` disables it.
...
| `relevantCanonChanged` *(per repo)* | a canon **pack this repo declares** moved, or a cross-cutting area (the engine/skills/migrations/bootstrap trees) moved | **growth-dedup-local-instructions** |
