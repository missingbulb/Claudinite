...
import { makeRepo, cleanup, writeFiles } from '../../engine-tests/checks/helpers.mjs';
...
// (The reasonless-accept-is-itself-a-finding half lives in engine-tests/checks/
...
test('runner integration: a reasoned accept excuses an isolation crossing', () => {
  ...
  try {
    ...
    const r = spawnSync(process.execPath, [join(canonRoot, 'engine', 'checks', 'run.mjs'), '--root', root], { encoding: 'utf8' });
