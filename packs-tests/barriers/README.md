...
Adopting the pack without a graph is a silent no-op, so adoption asks one question — what the barriers are *for* ([the adoption interview](../../packs/README.md#adoption-interview-questions)): the owner's answer is recorded verbatim as `answers.goals` on the entry, and the edge list below is distilled from it.
