...
import { parseOverrideBag } from '../../../../packs/sheepdog/tasks/fleet-add-missing-packs/worker.mjs';

// The parameter layer decides WHAT A RUN IS, from two call sites that cannot see each
// other — a command line in task.mjs and a free-form text box on a manual workflow run.
// Everything asserted here is a property whose loss would let one of them mean something
...
test('a boolean is `true` or `false` and nothing else', () => {
  ...
  assert.equal(parseParams({ argv: ['--repos=Alpha', '--scan-for-needed-packs=false'], overrides: { ADD_PACKS: 'p' } }).scan, false);
...
test('the override bag beats the weekly command line — that is what forcing IS', () => {
  const p = parseParams({
    ...weekly,
    overrides: { FORCE_TASKS: 'fleet-add-missing-packs', SCAN_FOR_NEEDED_PACKS: 'false', REPOS: 'Alpha Beta', ADD_PACKS: 'node' },
  });
...
test('FORCE_TASKS alone is not a force — it only says which task runs', () => {
  // A hand-started run of the ordinary weekly sweep must still report itself as the
  // ordinary weekly sweep; `forced` is about the PARAMETERS having been steered.
  assert.equal(parseParams({ ...weekly, overrides: { FORCE_TASKS: 'fleet-add-missing-packs' } }).forced, false);
});

test('lists are space-separated, because the override bag splits keys on commas', () => {
  // The coupling this pins: engine/scheduler/run.mjs's parseOverrides splits on `,` and
  // `\n`, so a comma inside a value silently becomes a second, unknown KEY. The worker
  // re-implements that shape (it is a subprocess reading a string), so assert the two
  // agree rather than trusting the comment.
  const raw = 'FORCE_TASKS=t,REPOS=Alpha Beta,ADD_PACKS=one two';
  assert.deepEqual(parseOverrideBag(raw), parseOverrides(raw));
  assert.deepEqual(parseOverrideBag(raw).REPOS, 'Alpha Beta');
  const p = parseParams({ argv: ['--scan-for-needed-packs=true'], overrides: parseOverrideBag(raw) });
  assert.deepEqual(p.repos, ['Alpha', 'Beta']);
...
test('a force must name its repos — the fleet keyword is refused', () => {
  ...
  assert.throws(
    () => parseParams({ argv: ['--scan-for-needed-packs=false', `--repos=${ALL_MEMBERS}`], overrides: { ADD_PACKS: 'node' } }),
    /refused/,
  );
});
...
test('config and answers for a pack nobody is adding are an error, not dead weight', () => {
  assert.throws(
    () => parseParams({ ...weekly, overrides: { REPOS: 'Alpha', ADD_PACKS: 'node', PACK_CONFIG: 'firebase.repo=o/r' } }),
    /not in ADD_PACKS/,
  );
});
...
test('two entries colliding on one key is an error, not a last-one-wins', () => {
  ...
  assert.throws(() => parseParams({
    argv: ['--scan-for-needed-packs=false', '--repos=Alpha'],
    overrides: { ADD_PACKS: 'p', PACK_CONFIG: 'p.repo=a/b', PACK_CONFIG_2: 'p.repo=c/d' },
  }), /conflict/);
});

test('a malformed entry names the shape it wanted', () => {
  const bad = (overrides) => () => parseParams({ argv: ['--scan-for-needed-packs=false', '--repos=Alpha'], overrides: { ADD_PACKS: 'p', ...overrides } });
