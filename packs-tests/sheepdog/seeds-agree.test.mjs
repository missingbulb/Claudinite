...
import rule from '../../packs/sheepdog/seeds-agree.mjs';
...
function ctx({ seed, own, declared = true }) {
  const packs = [
    { id: 'sheepdog', config: { owner: 'missingbulb', packSeeds: seed === undefined ? [] : [{ id: PACK, ...(seed === null ? {} : { config: seed }) }] } },
...
test('inert where it does not apply', () => {
  ...
  assert.deepEqual(
    ...
    [], 'no sheepdog config — not an enforcer, so there are no seeds');
