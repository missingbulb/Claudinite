...
// false-flag a repo that already tears down. Advisory: a missing pagehide is a
// smell to judge (an offscreen doc torn down via closeDocument is a valid
// exception to accept), not a proven runtime break. The skill's own directory is
// excluded so its fixtures never self-flag on the corpus repo; test/fixture
// files are excluded from the acquisition scan, where a mocked `getUserMedia`
// opens no real device.
