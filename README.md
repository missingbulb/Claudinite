...
- [routines/auto-all-repos-maintenance.md](routines/auto-all-repos-maintenance.md) — **the single scheduled entry point**, run daily from the fleet-**enforcer** repo (one that declares the [`sheepdog`](packs/sheepdog/README.md) pack). It runs the core planner over the repos it can reach, reads the emitted units, and runs each covered repo's due `run_daily` units at their `smarts` tier, honoring the growth barriers — no repo or unit can stop the others. Schedule **this**, nothing else.
...
- [packs/sheepdog/check-fleet-coverage.mjs](packs/sheepdog/check-fleet-coverage.mjs) — the enforcer pack's **census** (the cross-repo reach `sheepdog` adds): with an account-spanning PAT it enumerates the owner's repos, classifies coverage, converges one adoption issue per uncovered repo, and retires fully-applied baseline migrations. It is **coverage, not planning** — it does not build the work plan. Dispatched via that repo's `workflow_dispatch`-only coverage workflow (materialized from [the pack's stub](packs/sheepdog/stubs/fleet-coverage.yml)); which repos to cover and the exclude list come from its `packConfig.sheepdog`.
