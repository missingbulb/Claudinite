...
- `tasks/fleet-census/check-fleet-coverage.mjs` — the enforcer pack's **census** (the cross-repo reach the pack adds): with an account-spanning PAT it enumerates the owner's repos, classifies coverage, and converges one adoption issue per uncovered repo. It is **coverage, not planning** — it does not build any work plan, and it carries **no migration logic** (each member applies baseline migrations itself, via its own baselining's `engine/migrations/apply.mjs` run). It runs as that pack's ordinary agentless `fleet-census` task — the census **is** the task's `prework`, so it needs no workflow of its own; which repos to cover and the exclude list come from its pack entry's `config`.
