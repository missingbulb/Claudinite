...
`engine/migrations/` holds the **baseline migrations** mechanism — declared path relocations, one record per canon rename (a renamed or relocated artifact consumers hold their own copy of) that supplies the read-side resolver and the write-side rename. Each record lives under the flow that owns it: an engine change beside the mechanism, a pack's own under `packs/<pack>/migrations/`. Every record ever landed stays there as durable history; **fetching decides relevance** — vendoring ships consumers only the records landed within the last 7 days, while a dormant project baselining out of a fresh canon clone applies whatever it missed. Named for when it runs: baselining applies each. See [engine/migrations/README.md](engine/migrations/README.md).

Scheduled work is **per-repo**, not central. `engine/scheduler/` ([design](docs/per-project-scheduling/DESIGN.md)) runs in each repo's own workflow off that repo's `taskScheduler` anchor: it discovers its declared packs' `tasks/<name>/` folders structurally, evaluates each task's precondition in code, and dispatches the due ones. **Baselining** is the baseline pack's task, **growth** rides the repo-side pack and the home-side pack, and the PR/branch/issue tidy-up is three per-dimension tasks of the tidy pack — each a `tasks/<name>/` under its pack.

- `tasks/fleet-roster/check-fleet-roster.mjs` — the enforcer pack's **roster sweep** (the cross-repo reach the pack adds): with an account-spanning PAT it enumerates the owner's repos once and answers two questions from that one walk — is each repo covered (converging one adoption issue per uncovered repo) and is each covered member still keeping up with canon (converging one drift issue per member that isn't). It is **classification, not planning** — it does not build any work plan, and it carries **no migration logic** (each member applies baseline migrations itself, via its own baselining's `engine/migrations/apply.mjs` run). It runs as that pack's ordinary agentless `fleet-roster` task — the sweep **is** the task's `prework`, so it needs no workflow of its own; which repos to cover and the exclude list come from its pack entry's `config`.
