...
- [routines/auto-all-repos-maintenance.md](routines/auto-all-repos-maintenance.md) — **the single cross-repo entry point.** One daily routine, scheduled once from a home repo, that discovers **every** Claudinite-vendored repo the token can access (by the tracked `.claudinite/.gitkeep` marker) and dispatches each as an isolated subagent running its own consolidated daily maintenance. Schedule **this** once to cover a whole fleet, instead of registering the per-repo routine below in each repo. Sits one level above `auto-daily-maintenance.md` and composes with it.
- [routines/auto-daily-maintenance.md](routines/auto-daily-maintenance.md) — **the single per-repo entry point.** One daily routine that dispatches every daily maintenance routine below as its own isolated subagent, so a repo's upkeep is one schedule and all members are guaranteed to run each day. Schedule **this**, not the members individually — or, to cover many repos at once, let the fleet routine above dispatch it per repo.
