...
- [routines/auto-fleet-coverage.md](routines/auto-fleet-coverage.md) — the **fleet-coverage sweep**: every repo under the owner's account gets Claudinite adopted into it (the idempotent bootstrap, run unattended) unless it already mounts the corpus or is named on the owner-maintained [opt-out list](routines/fleet-coverage-opt-out.md), so the account converges to covered-or-opted-out. Sequenced by the daily routine above — never scheduled itself.
