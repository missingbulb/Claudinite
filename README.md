...
Scheduled work is **per-repo**, not central. `packs/claudinite-tasks/` ([design](packs/claudinite-tasks/docs/per-project-scheduling/DESIGN.md)) runs in each repo's own workflow off that repo's `taskScheduler` anchor: it discovers its declared packs' `tasks/<name>/` folders structurally, evaluates each task's precondition in code, and dispatches the due ones. **update** is the baseline pack's task, **growth** rides the repo-side pack and the home-side pack, and the PR/branch/issue tidy-up is three per-dimension tasks of the tidy pack — each a `tasks/<name>/` under its pack.
