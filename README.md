...
Scheduled work is **per-repo**, not central. `engine/scheduler/` ([design](docs/per-project-scheduling/DESIGN.md)) runs in each repo's own workflow off that repo's `taskScheduler` anchor: it discovers its declared packs' `tasks/<name>/` folders structurally, evaluates each task's precondition in code, and dispatches the due ones. **Baselining** is the baseline pack's task, **growth** rides the repo-side pack and the home-side pack, and the PR/branch/issue sweep is the repo-tidy pack's — each a `tasks/<name>/` under its pack.
