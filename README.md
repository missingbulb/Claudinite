...
- [routines/auto-fleet-bootstrap.md](routines/auto-fleet-bootstrap.md) — the **fleet bootstrap sweep**, owning all bootstrap work across the fleet: every member gets its mount wiring **re-bootstrapped** and the repo **aligned** with its declared packs' current checks (delivered per the member's `maintenance.delivery` flag in `.claudinite-checks.json` — direct push by default, a never-merged PR on request), and every repo under the owner's account that mounts nothing gets **adopted** via the same idempotent bootstrap — unless named on the owner-maintained [opt-out list](routines/fleet-bootstrap-opt-out.md) — so the account converges to covered-or-opted-out. A daily deterministic **coverage census** ([fleet-coverage.yml](.github/workflows/fleet-coverage.yml) running [check-fleet-coverage.mjs](routines/check-fleet-coverage.mjs) with an account-spanning PAT) keeps that knowledge honest, converging one adoption issue per unwired, not-opted-out repo. Sequenced by the daily routine above — never scheduled itself.
