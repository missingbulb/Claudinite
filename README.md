...
- The growth pack — the **member-side** stages, on every repo declaring the pack: **extract** (captures the last 24h of bugs/PRs/commits into the project's **own** docs, committing directly to its `main`), **dedup** (prunes local items the canon covers, keeping items the canon states too generally; opens a PR), and the weekly **pack discovery** pipeline.
