...
`migrations/` holds the **baseline migrations** mechanism — declared path relocations, one record per canon rename (a renamed or relocated artifact consumers hold their own copy of) that supplies the read-side resolver and the write-side rename. Every record ever landed stays there as durable history; **fetching decides relevance** — vendoring ships consumers only the records landed within the last 7 days, while a dormant project baselining out of a fresh canon clone applies whatever it missed. Named for when it runs: baselining applies each. See [migrations/README.md](migrations/README.md).
