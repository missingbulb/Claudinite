...
- [routines/auto-fleet-bootstrap.md](routines/auto-fleet-bootstrap.md) — the **fleet bootstrap sweep**, owning all bootstrap work across the fleet: every member gets its mount wiring **re-bootstrapped** (drift refresh, committed directly), and every repo under the owner's account that mounts nothing gets **adopted** via the same idempotent bootstrap — unless named on the owner-maintained [opt-out list](routines/fleet-bootstrap-opt-out.md) — so the account converges to covered-or-opted-out. Sequenced by the daily routine above — never scheduled itself.
