...
- [routines/auto-all-repos-maintenance.md](routines/auto-all-repos-maintenance.md) — **the single scheduled entry point**, run daily from the fleet-**enforcer** repo (one that declares the enforcer pack). It runs the core planner over the repos it can reach (the home repo included, planned last), reads the emitted units, and runs each covered repo's due units at their `smarts` tier — no repo or unit can stop the others. It is the **rollback path** for the per-project scheduling migration (#394) and covers only repos that have not yet cut over: the canon packs' own tasks are now per-repo scheduler tasks (`packs/<pack>/tasks/<name>/`), so the planner assembles nothing from them.
- [routines/fleet/plan.mjs](routines/fleet/plan.mjs) — the **planner**: pack-agnostic core code that goes over the repos it can reach and, for each covered member, assembles its declared packs' legacy `run_daily` tasks, runs each gate in code, and emits the day's work plan. It depends on no single pack — an enforcer pack's coverage census is a separate, isolated concern, never the source of the plan. Since the canon packs' `run_daily/` folders retired, the only tasks left for it to assemble are an unmigrated repo's own local-pack ones.
- `check-fleet-coverage.mjs` — the enforcer pack's **census** (the cross-repo reach the pack adds): with an account-spanning PAT it enumerates the owner's repos, classifies coverage, and converges one adoption issue per uncovered repo. It is **coverage, not planning** — it does not build the work plan, and it carries **no migration logic** (baseline-migration application and retirement are the migrations flow's own standalone passes, `migrations/fleet-apply.mjs` + `migrations/fleet-retire.mjs`). It runs as that pack's ordinary agentless `fleet-census` task — the census **is** the task's `agent_preprocessing`, so it needs no workflow of its own; which repos to cover and the exclude list come from its pack entry's `config`.
- [routines/fleet/](routines/fleet/DESIGN.md) — the **legacy central task engine** (the code that decided each repo's due tasks from its active packs) and its scheduling contract, retained as the migration's rollback. The live engine is `engine/scheduler/` ([design](docs/per-project-scheduling/DESIGN.md)), which runs in each repo's own workflow: **baselining** is the baseline pack's task, **growth** rides the repo-side pack and the home-side pack, and the PR/branch/issue sweep is the repo-tidy pack's — each a `tasks/<name>/` under its pack.
