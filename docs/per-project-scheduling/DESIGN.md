...
| growth-extract (grow_with_claudinite) | daily-1h | commits, prs, issues | opus | merged-pr | Precondition = substantiveChange; context = the commit/PR/issue lists. **As built: one task over both sources.** Designed (and first built) as two tasks — `growth-extract` over activity, `conversation-extract` over the captured logs — firing in the same 03:00 slot against the same local packs. They share the lesson bar, the promotion ladder and the dedup surface, so the split cost a second opus dispatch, a second PR, and two runs deduping against a corpus the other was concurrently writing. The worker now runs the `extract-from-activity` and `extract-from-conversations` skills in turn, then `prose-to-checks` over the prose it just wrote, and lands all of it in one auto-merging PR. **The retention prune left (#964)**: it was a second precondition arm whose only job was to fire the prune on a repo with nothing to extract, so it woke an opus dispatch for date arithmetic on an orphan branch. It is `logs-prune` below, and the coupling that made it judgment work — an aged log got a final pass before deletion — survives as a marker this run writes (`<log>.processed`) and the prune reads. |
| logs-prune (grow_with_claudinite) | daily | conversationLogs | none | none | Retention on the `conversation-logs` branch, split out of `growth-extract` (#964). Agentless: what to delete is arithmetic on the capture stamps, and the write is remove commits on a non-default branch (outside the outcome taxonomy, §1). It deletes only a capture that is BOTH past `retention_days` AND marked `<log>.processed` by an extract run — age is this task's to judge, consumption is not — and reports how many aged-but-unmarked captures it is holding, so a repo whose extract stopped running does not read as a quiet one. |
