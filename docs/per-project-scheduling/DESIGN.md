...
| create-extractor (gcec, local) | **hourly** | issues | sonnet | open-pr | **Revised by agent-preprocessing (§9), as built.** The *precondition* is only the cheap gate it can be from signals alone: is any open `extractor-request` issue eligible (not already blocked or in flight)? Everything deterministic — triage over the committed sources' `matches()`/`classifyHost`, closing deny/allow/duplicate with the canned message, name computation, branch + scaffold, **the ScraperAPI page fetch** (its secret declared in `agent_preprocessing_secrets`, which is what retires `fetch-page.yml`), and the draft PR — is `agent_preprocessing`, which needs the I/O and writes a precondition cannot do. The agent is requested (conditional hand-off) only when a real page is recorded and an `extract()` is left to write. The user-facing `extractor-request` issue stays; the dispatch issue references it. |
