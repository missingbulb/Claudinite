...
| tidy-issues (tidy-repo) | daily | issues, commits | sonnet | none | The undeclared-canon carve-out dies: the canon repo declares tidy-repo like everyone else. **One task per tidy dimension** (the single `repo-tidy` pass split, #481): the acting dimension. Scope = the touched issues, widened to every open issue on a substantive default-branch move — that move is what can make an old issue implemented, so the "full sweep" is signal-triggered, never a calendar flag. |
| tidy-prs (tidy-repo) | weekly | prs | sonnet | none | Assess-only. Full every run (scope = every open PR): a PR verdict is a standing recommendation, not a same-day alert, so the full sweep is the **frequency declaration** — consistent with `fullSweep` retiring in §3. |
| tidy-branches (tidy-repo) | weekly | branches | sonnet | none | Assess-only. Full every run: the `branches` signal carries names only, so there is no windowed subset to narrow to, and branch cruft accumulates on a weekly clock. Excludes the presumed default names and the infra branches (`conversation-logs`, `claudinite/maintenance`); the worker owns excluding the repo's *real* default branch. |
