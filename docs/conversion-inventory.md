...
| **chrome-extension-release.md — the contract** | **check** `chrome-extension-release/` conformance suite: the single `Release to Chrome Store` workflow stub (file `chrome-extension-release.yml`, named `Release to Chrome Store`, daily schedule at the contract cron, calling the three canon reusable workflows) · no surviving `__TOKENS__` · required `.github/release.config` with all 5 explicit keys, no defaults · manifest ↔ `package.json` version sync · zip forced to `dist/<kebab repo>.zip` (derived) · README Install/Releasing sections · `PRIVACY.md` present · every manifest permission disclosed in `PRIVACY.md` (test-the-world) · an *added* permission raises an advisory to open the store-dashboard justification issue (test-the-work) · release machinery under `dev/build/release/` · store-asset inventory (128px icon, ≥1 screenshot) · assets generator-backed. The patch-bumper and shipped-paths filter are now canon composite-action/workflow logic, not per-repo scripts. Listing copy + permission justifications are Chrome Web Store dashboard state (no repo file); first publication is a one-time tracking issue, not a `STORE-LISTING.md`; no per-repo `releasing.md`. Doc keeps the setup narrative and manual store steps |
