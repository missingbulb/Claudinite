...
- **Open a single PR against `main`** — one PR for the whole run's prunes, not one per item — never a direct push, on a per-run-unique branch (not a bare date) so a same-day re-run doesn't collide (canon: [an automated job needs a unique branch per run](../tasks/git-and-github.md#an-automated-job-needs-a-unique-branch-per-run)). This is an unattended routine, on a capable model, editing the project's *own* docs; the owner wants a human approval gate on every growth change.
