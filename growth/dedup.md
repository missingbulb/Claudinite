...
- **The project's local docs.** Identified the same way in every phase: the docs reachable by following the pointer graph from the repo's root `CLAUDE.md`, **minus** the mounted canon under `.claudinite/` (see [growth/README.md](README.md)). That `CLAUDE.md`-anchored set is the corpus this phase prunes within — the canon under `.claudinite/` is never a prune target, only the yardstick you prune *against*.
