...
- **[extracting-lessons.md](extracting-lessons.md)** — the method for mining a single working session for lessons: the owner's on-demand "learned lessons" command, and the pass the merge flow runs after every merge. It's the content the `lessons-learned` skill carries (invoked by the `merge-to-main` skill); delivered as a PR for review — the same approval gate the daily phases use.
