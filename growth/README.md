...
- **[extracting-lessons.md](extracting-lessons.md)** — the **on-demand** counterpart to phase 1: the method for mining a single working session for lessons when the owner triggers it by hand. Unlike the daily phases it's delivered as a PR for review, not committed straight to `main`.
