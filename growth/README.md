...
- **[promote.md](promote.md)** — phase 2, central. Reads every project's local docs, **generalizes** the portable lessons, routes each to the right canon home, and opens a PR against Claudinite's `main`. When a portable lesson's technology has **no pack home**, it mints a fingerprinted **stub pack** for it (in its own dedicated PR), so the next project on that stack self-declares instead of re-deriving. This is the sole judgment gate before shared canon.
