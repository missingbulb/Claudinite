...
- **Never mint a pack from imagination** — a stub is minted only to give a *real, portable lesson* a home, and it carries only what distilled from that lesson (or an honestly-empty `RULES.md` when the fingerprint is worth landing ahead of the prose). Do not pre-populate it with rules no project earned; that is the "written from imagination, not distilled from worked examples" failure the stubs exist to avoid.
