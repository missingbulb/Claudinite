...
- **Bounded write surface — `packs/` and `skills/` only.** Every file this routine creates or edits lands under `packs/<name>/` or `skills/<name>/`, the two homes of the shared canon, and **nowhere else** in the repo. The engine and orchestration (`checks/` lib, `growth/`, `routines/`, `migrations/`, the top-level docs, `bootstrap.md`) and the per-user `preferences/` are all off-limits: a promoted lesson is portable canon, so it belongs in a pack's prose or checks, or in a skill — never in orchestration code or a personal preference file (this overrides [item-routing.md](item-routing.md)'s per-user routing group, which the on-demand paths still use but promote does not). Within that boundary, edit only the one owning doc — or, for a check-conversion, only the new rule module, its registration in the pack registry, and its fixture test. Don't "improve" unrelated rules or checks while you're in there. Adding a new corpus file is the other exception, via the new-doc path [item-routing.md](item-routing.md) owns — and it too stays inside `packs/`/`skills/` (a new pack directory or a new skill). A worthy lesson that genuinely can't be homed within that boundary is **outside this routine's scope**: leave it local and flag it, never reach past the boundary to place it.
- **Certify the boundary before opening the PR — the prose above is the request, this gate is the guarantee.** With the branch committed and *before* opening the PR, run the diff-scope gate over the branch's own working tree: `node growth/promote-scope.mjs`. It scopes the change against the merge-base with `main` and exits non-zero if any added, modified, or deleted path lies outside `packs/`/`skills/` (exit 1 = a stray path, exit 2 = no merge-base to scope against). A non-zero exit means the boundary was breached — **do not open the PR**; move the stray edit inside `packs/`/`skills/` or drop that lesson as local, then re-run the gate until it certifies clean. (This is the [checks philosophy](../checks/DESIGN.md) applied to the routine's own output — "Prose is a request; the post-hoc diff check is the guarantee." The gate is invoked *by* this routine rather than registered as an always-on check because "packs/skills only" is true for promote alone: an ordinary engine change legitimately edits `growth/`, `checks/`, `routines/`, and nothing in the tree marks a diff as a promote run.)
...
- **Never write outside `packs/` and `skills/`.** The routine's entire write surface is the shared canon's two homes — `packs/<name>/` and `skills/<name>/`. Engine and orchestration code (`checks/` lib, `growth/`, `routines/`, `migrations/`, the top-level docs, `bootstrap.md`) and the per-user `preferences/` are never touched by a promotion; a lesson that can only live outside that boundary isn't promotable here — leave it local.
