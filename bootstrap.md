...
The corpus ships a set of unattended daily routines for a consuming repo — a lessons digest, a local-docs/canon reconciliation, a nightly branch-status report (the `routines/` specs). You don't wire these up one by one, and you don't wire them up **per repo**. Schedule **a single routine** — [`.claudinite/routines/auto-all-repos-maintenance.md`](routines/auto-all-repos-maintenance.md) — once, from one home repo: it discovers every Claudinite-vendored repo the token can access (by the tracked `.claudinite/.gitkeep` marker) and dispatches every member routine directly, per repo, as its own isolated subagent, so one scheduled job guarantees all of them run in every opted-in repo each day and a failure in one member or one repo can't stop the others.

To adopt: register **one** daily schedule (the Claude Code Routines UI, a cron, or a CI nightly trigger) from your home repo, whose prompt is the thin pointer at the bottom of that doc, substituting the path where `.claudinite/` is mounted. Do **not** also schedule the member routines individually, and do **not** register a separate per-repo maintenance schedule — either would double-run them. Adding a future daily routine later means adding one line to that doc's member list, not registering another schedule.
