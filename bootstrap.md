...
**A consuming project schedules nothing.** The corpus ships a set of unattended daily routines — a lessons digest, a local-docs/canon reconciliation, a nightly branch-status report (the `routines/` specs) — but maintenance is run **centrally, once**, from the owner's home repo by the fleet routine [`routines/auto-all-repos-maintenance.md`](routines/auto-all-repos-maintenance.md). That routine discovers every Claudinite-vendored repo the token can access — detected by exactly the tracked `.claudinite/` signal this repo already carries from mounting the corpus above (the `.claudinite` submodule registration under Method A, or the committed `.claudinite/.gitkeep` marker under Method B) — and runs every member routine against each, per repo, in isolation.

So mounting the corpus **is** the opt-in: once this repo carries that signal, the fleet routine picks the repo up automatically and it gets the maintenance goodies for free. There is no per-repo schedule to register, no member routines to wire up, and nothing to run here. (Setting up that one central schedule is the home repo's job, not a consumer's — see the fleet routine's own doc.)
