...
Because promotion into the canon now runs **centrally** from the home repo (which already holds a fleet-wide token), there is **no per-repo up-path to install** — no handoff GitHub Action, no Claudinite-scoped PAT secret, no handoff label. Those are gone. So mounting the corpus is nearly the whole opt-in: once this repo carries that signal, the fleet routine picks it up for free. There is no per-repo schedule to register and no plumbing to wire up. (Setting up that one central schedule is the home repo's job, not a consumer's — see the fleet routine's own doc.) The **one** action bootstrap takes for maintenance is filing the enrollment issue below.

**Bootstrap files the enrollment issue itself — this is an automated bootstrap step, not a human chore.** The fleet routine reaches only repos on **its access list — an explicit per-repo allowlist the fleet owner maintains in the routine's UI**, not an org-wide token — so a freshly mounted repo isn't enrolled until the owner adds it there, and only the owner can. To make sure that owner-side step is never forgotten, **running these bootstrap steps creates** a GitHub issue in this repo's tracker, **assigned to `missingbulb`** (the fleet owner), asking to add this project to the daily-maintenance routine's repo-access list. The bootstrapping agent files it as part of bootstrap — no person has to remember to open it. Keep it **idempotent** like everything else here: search the tracker first and skip if such an issue (open or closed) already exists, so a re-bootstrap never files a duplicate.
