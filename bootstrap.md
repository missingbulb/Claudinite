...
**The one bootstrap action: open a tracking issue for the fleet owner.** Auto-discovery is the real opt-in, but it only reaches repos the **central token can see** — the fleet routine enumerates "every repo the token can access" and nothing more, so a freshly bootstrapped repo in a scope that token can't yet reach is silently dropped from the day's run. To close that gap, bootstrapping opens a GitHub issue in **this** repo's tracker, **assigned to `missingbulb`** (the Claudinite fleet owner), asking to add this project to Claudinite daily maintenance — the human bridge that guarantees the owner knows to confirm coverage rather than trusting silent discovery. Keep it **idempotent** like the rest of bootstrap: search the tracker first and skip if such an issue (open or closed) already exists, so a re-bootstrap never files a duplicate.
