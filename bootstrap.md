...
- **Preferences** — the owner's per-user interaction preferences live in the canon's `preferences/<email>.md`, per-user content a project never owns. `.claudinite/mount/inject-preferences.sh` expands `CLAUDE_CODE_USER_EMAIL` and prints the matching file (which the orchestrator forwards to the session context) — read from the local tree when it carries one, fetched as a single file over HTTPS otherwise. **Fail-soft by design**: a miss — no `CLAUDE_CODE_USER_EMAIL`, no matching `<email>.md`, a failed fetch — injects a one-line note and the session proceeds on defaults; the halt-gate below is reserved for the load-bearing corpus and environment, not this nice-to-have.
- **Active-pack prose** — every pack the project declares in `.claudinite-checks.json` carries its guidance as `RULES.md` prose, including the baseline (working discipline, the task lifecycle): **no pack is active by default** — Part 6's `--init` seeds the baseline declaration and its backfill step adds it to a pre-existing file. `.claudinite/packs/load-active-prose.mjs` emits the active packs' prose each session. **Without it, declaring a pack has no effect** — the `@.claudinite/CLAUDE.md` import pulls only the corpus *index*, never a pack's prose, not even the baseline. This is what Part 5's "its prose then loads every session" relies on.

> **The halt-gate capability.** A SessionStart hook **cannot** block the session or prompt interactively — no exit code halts session start (exit 2 doesn't block, and worse, it makes Claude Code **discard the hook's stdout**, the very channel a step would use to speak up). But on a normal (exit 0) hook, **stdout is injected into the session context**. So when a step can't do its job, instead of failing silently it prints a **plain-text** directive telling the assistant to **STOP and use `AskUserQuestion`** before doing any work. The assistant carries out the confirmation the hook itself can't, turning an un-blockable hook into an effective, in-your-face gate. `sync-claudinite.sh` (sync failed, no local copy) and `env.mjs check` (a prerequisite is missing) both use exactly this. The message is **plain text, not a JSON envelope**: the orchestrator forwards every step's stdout through one hook, and one hook's stdout that mixes JSON and prose parses as neither — so each step emits plain text that simply concatenates.
...
The portable merge recipe ships as the merge skill and needs **nothing** from you — its default is squash-merge via a PR, gating on CI only when the repo has it. Adopt it and you're done.
...
The owner runs recurring **classes** of project, each carried by a **project-class pack** (a prose-only pack a project declares, no fingerprint — e.g. the algorithm-iteration playbook). Categorizing a new project is just declaring the pack that fits, alongside its technology packs from Part 3's `--init`:
...
**3.** Write the initial pack declaration — the baseline plus the repo's technology fingerprint (skips itself if the file already exists):
...
The web base image is minimal — it ships no toolchain SDKs, a repo's `npm` modules aren't cloned, etc. Install belongs in the environment **image** (built once, then Anthropic snapshots the filesystem and reuses it), NOT a per-session hook that would reinstall every start. The division of labour:
...
- **A session-start step only asserts.** `env.mjs check` *probes* each requirement directly (a toolchain binary on PATH, `node_modules` present) and, if one is missing, prints the halt-gate directive telling the assistant to have you re-paste. It never installs, and there is no version flag — the probes are the source of truth.
...
**2.** Give the packs any per-repo parameters they need as `config` on the pack's entry in `.claudinite-checks.json` — e.g. where a pack should run `npm ci` (default: repo root):

```json
{ "packs": [ { "id": "<pack>", "config": { "dirs": ["packages/api"] } } ] }
```

**3.** Apply the setup to the environment: copy the full body of **`.claudinite/mount/environment-setup.sh`** (present after a corpus sync; or copy it from the Claudinite repo) into the web environment's **Setup script** field (web UI → environment selector → edit environment → Setup script), then start a fresh session so the snapshot rebuilds. The network policy must reach whatever the active packs' setup fetches (e.g. `github.com`, `storage.googleapis.com`, `pub.dev`, or the npm registry).
