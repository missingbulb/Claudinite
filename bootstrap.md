...
**A consuming project schedules nothing, and wires up no plumbing.** The corpus ships the unattended [growth lifecycle](growth/README.md) — per-project lesson **extract**, central **promote** into the canon, per-project **dedup** — plus a nightly branch-status report, but it is all run **centrally, once**, from the owner's home repo by the fleet routine [`routines/auto-all-repos-maintenance.md`](routines/auto-all-repos-maintenance.md). That routine discovers every Claudinite-vendored repo the token can access — detected by exactly the tracked `.claudinite/` signal this repo already carries from mounting the corpus above (the `.claudinite` submodule registration under Method A, or the committed `.claudinite/.gitkeep` marker under Method B) — and sequences the lifecycle across the fleet, each phase per repo in its own isolated subagent.

Because promotion into the canon now runs **centrally** from the home repo (which already holds a fleet-wide token), there is **no per-repo up-path to install** — no handoff GitHub Action, no Claudinite-scoped PAT secret, no handoff label. Those are gone. So mounting the corpus **is** the entire opt-in: once this repo carries that signal, the fleet routine picks it up automatically and it gets the maintenance goodies for free. There is no per-repo schedule to register, nothing to wire up, and nothing to run here. (Setting up that one central schedule is the home repo's job, not a consumer's — see the fleet routine's own doc.)

**The one bootstrap action: open a tracking issue for the fleet owner.** Mounting the marker is only *half* the opt-in. The fleet routine can only reach repos on **its own access list — an explicit per-repo allowlist the fleet owner maintains in the routine's UI**, not an org-wide token — so it never even *sees* a freshly bootstrapped repo until the owner adds it there by hand. That step only the owner can do, so bootstrapping opens a GitHub issue in **this** repo's tracker, **assigned to `missingbulb`** (the Claudinite fleet owner), asking to add this project to the daily-maintenance routine's repo-access list. Until that's done the marker sits unread — the issue is what drives the owner-side step that actually enrolls the repo. Keep it **idempotent** like the rest of bootstrap: search the tracker first and skip if such an issue (open or closed) already exists, so a re-bootstrap never files a duplicate.
