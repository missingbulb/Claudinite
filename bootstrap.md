...
**One bootstrap step remains: file the enrollment issue.** The fleet routine only maintains repos on its access list — a per-repo allowlist the owner maintains in the routine's UI — so mounting the marker doesn't enroll a repo until the owner adds it there. So as part of bootstrap, open a GitHub issue in this repo's tracker **assigned to `missingbulb`**, asking to add the project to that list. Idempotent: search first, skip if one (open or closed) already exists.
