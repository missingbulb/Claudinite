...
- **A local pack** — `.claudinite/local/packs/<name>/` in a **consumer's own tree**, tracked
  ...
  **normalized capture surface** (what used to sprawl as always-loaded `CLAUDE.md`/`dev/procedures`
  ...
  (the bare id stays accepted permanently; the pre-rename `local_packs/<name>` token and its
  `.claudinite/local_packs/` root are both retired; [packs/README.md](packs/README.md#local-packs--a-projects-own-packs)). Prose injection, the Stop/CI checks, skill mounting, and
