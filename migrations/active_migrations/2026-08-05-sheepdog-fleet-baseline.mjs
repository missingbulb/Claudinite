...
//
// This record is what makes "edit the pack, not the copy" true. The apply pass
// re-materializes the file every cycle, so a hand-edited or stale copy self-heals and a
// canon fix reaches the enforcer the night it lands.
//
...
// enforcer pack IS the adoption here, so the first apply pass after the declaration is
// what installs the workflow, with no separate bootstrap step. Claudinite itself does not
// declare sheepdog, so the canon never self-applies.
...
export default {
  ...
  // A `packs` entry is either the bare id or `{ id, ... }` — both forms are legal in a
  // declaration, so both are matched. An unreadable or unparsable declaration means "not
  // an enforcer as far as this record can tell": skip, never guess a repo into hosting a
  // fleet-wide lever.
