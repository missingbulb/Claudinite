## 2026-08-24 · born · Cut the local pack's RULES.md to one trigger and directive per rule (#1315)
- **Reason:** split from landing a change to what members receive, which fires before you have
  decided to force at all.
- **Actor:** @missingbulb (owner).
- **Model:** Claude Opus 5, per the commit trailer.
- **Mechanism:** a RULES.md rule, triggered on "Forcing fleet delivery".
- **Landed:** #1315 (Closes #1312).

## 2026-09-25 · reworded · the lever's id is now fleet-update
- **Reason:** the sheepdog lever was renamed from fleet-baseline.
- **Actor:** @missingbulb (owner).
