## 2026-07-26 · born · Claudinite growth: extract lessons (#436)
- **Reason:** #907: the nightly refresh recomputes the vendor set from canon HEAD, so a pattern bug
  there is the one canon regression that does not heal itself, and a fixture cannot see it.
...
- **Reason:** the whitelist test asserted the one file that had broken rather than its directory
  (#907).
- **Actor:** @missingbulb (owner).
