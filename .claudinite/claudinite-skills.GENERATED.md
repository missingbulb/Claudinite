...
| `packs/*/skills/**` | `writing-claudinite-skills` | claudinite-canon-curation | Authoring or changing a skill in the Claudinite corpus: SKILL.md conventions, where its checks and tests live, pack wiring. Use when creating or editing a canon pack's skills/<name>/. |
...
| `extract-packs-from-a-project` | claudinite-canon-curation | Decompose a project into its pack facets and author the canon packs they earn. Use from a canon when a project shows a facet its shelf does not yet home. |
...
| `running-the-suite` | claudinite | Running this repo's test suite: the one command that covers it, and reading a run's output from a file. Use before any node --test. |
