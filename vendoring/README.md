...
| File | Role |
|---|---|
| [`compute-vendor-set.mjs`](compute-vendor-set.mjs) | The **vendor-set computation**: a repo's pack declaration → the corpus file set that repo persists under `.claudinite/shared/` (the `engine/` root minus tests and root docs, plus the declared packs with their `requires` closure, each pack's bundled skills riding its own tree). Verified import-closed before it is returned. Also where a set file's *content* comes from (`vendoredContent`, landed by `copyIntoMount`): the canon's bytes, except that a pack's rule prose and guidelines skills cross without their provenance markers. |
