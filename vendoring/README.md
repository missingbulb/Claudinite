...
| [`../engine/hooks/session-start-command.sh`](../engine/hooks/session-start-command.sh) | The **orchestrator**. Runs the corpus-dependent session-start steps **in sequence, in one process** (Claude Code runs hook entries in parallel, so ordering can't live across sibling entries). Forwards each step's stdout to the session context and logs a timeline to `.claudinite-hooks.log`. Method A / the canon repo invoke it directly; Method B reaches it through `sync-claudinite.sh`. |
| [`../engine/hooks/environment-setup-command.sh`](../engine/hooks/environment-setup-command.sh) | The generic **cloud-environment setup** — pasted once into a web environment's Setup script field. Primes the corpus and runs each active pack's `env.mjs install`. Identical for every project, so a consumer commits no copy of its own. |
| [`compute-vendor-set.mjs`](compute-vendor-set.mjs) | The **vendor-set computation** for the incoming tracked mount ([DESIGN.md](DESIGN.md)): a repo's pack declaration → the minimal corpus file set that repo persists under `.claudinite/shared/`. Not yet wired into the live mount. |
| [`../engine/hooks/steps/inject-preferences.sh`](../engine/hooks/steps/inject-preferences.sh) | The **preferences step**: injects the current user's `preferences/<email>.md` — local copy first, single-file HTTPS fetch otherwise (preferences are per-user and never vendored). Fail-soft: any miss is a one-line note, never a halt. |
...
- `engine/pack_loader/inject-pack-prose.mjs` — emits the active packs' prose.
- `engine/pack_loader/mount-skills.mjs` — (re)generates the skill mounts.
- `engine/pack_loader/env-requirements.mjs check` — the cloud env-requirement assertion.

The enforcement hooks stay in `checks/` (`stop-hook.mjs`, `pretooluse-guard.mjs`) and the shared
logger in `engine/checks/helpers/hook-log.mjs`. `mount/` is the *loading* story; those are separate concerns.
