...
- **Source:** the standard missingbulb/GoogleCalendarEventCreator had just adopted (its #678) - the
  ...
  a dated growth log.
- **Reason:** without the sections, compile-once/refine-in-place degrades to unsourced, unreviewable
  rewrites nobody can read at a glance.
- **Actor:** @missingbulb (owner).
- **Model:** Claude Sonnet 5, per the commit trailer.
- **Mechanism:** a coded check over every wiki page's required headings, blanking fenced content
  first so an embedded template is neither a section nor a log.
- **Landed:** #301 (Refs #302) · pack version 1.

## 2026-07-29 · severity-changed · product-wiki: every wiki page opens with a Key insights header (#542)
...
- **Mechanism:** the required-section list gains the header, so a missing header is reported exactly
  once, where every other missing section is, and never double-reported by the check that owns the
  header's shape.
