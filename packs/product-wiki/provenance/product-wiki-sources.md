...
- **Source:** the standard missingbulb/GoogleCalendarEventCreator had just adopted (its #678) - the
  ...
  a dated growth log.
- **Reason:** the invariant is reachability, not link typography, so a bare URL verifies and a
  bullet is judged over its whole wrapped block - a hard-wrapped source whose URL lands on a
  continuation line was a blocking false positive.
- **Actor:** @missingbulb (owner).
- **Model:** Claude Sonnet 5, per the commit trailer.
- **Mechanism:** a coded check over the Sources section's bullets.
