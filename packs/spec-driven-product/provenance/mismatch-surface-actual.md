...
- **Reason:** an agent that adjusts the expected has quietly transferred ownership of the product to
  itself - the strongest statement of why a mismatch is surfaced and asked about rather than
  absorbed.
- **Actor:** @missingbulb (owner).
- **Model:** Claude Fable 5, per the commit trailer.
- **Mechanism:** a RULES.md rule, triggered on "On a mismatch, surface actual vs. expected (and the
  diff) and ask.".
...
- **Retire when:** reaffirm while an agent can write the expecteds; retire only if they are made
  unwritable to it.
