## 2026-08-12 · born · Claudinite growth: discover canon pack web-scraping (#739)
- **Source:** EdFringeNow's local `edfringe-data` pack, whose cross-the-zone-once rule carried it; a
  reviewer in the developer's own zone cannot see the difference, which is how a device clock
  survives review.
- **Actor:** @missingbulb (owner).
- **Model:** Claude Sonnet 5, Claude Opus 5, per the commit trailer.
- **Mechanism:** a RULES.md rule, triggered on "Taking a "now"" - separated from the timestamp
  conversion because a reader can arrive at it without that rule.
- **Landed:** #739 (Refs #717) · pack version 1.
