...
description: Widget-test and golden mechanics in Flutter. Use when writing or debugging a widget test or golden, or when goldens render as boxes, hang, or drift.
metadata:
  body: guidelines
  usage:
    expect: triggered
...
- **Load real fonts before any golden** — the test binding defaults to the glyph-less Ahem stub
  ...
  leak; pin `fontFamily` there explicitly. (load-real-fonts)
...
- **Never `pumpAndSettle` around indeterminate progress indicators** — they schedule frames
  ...
  (pumpandsettle-around-indeterminate)
...
- **Fix the viewport per suite**: set `tester.view.physicalSize` and `devicePixelRatio` to one
  ...
  the harness default. (fix-viewport-suite)
