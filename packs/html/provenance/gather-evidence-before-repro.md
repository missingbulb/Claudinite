## 2026-09-24 · born · given a file of its own from an unmarked section
- **Source:** the unmarked RULES.md section "Get the browser's own evidence before guessing a repro
  for a live-data bug report", whose history is in git before this date.
- **Reason:** the owner asked to trim rule prose to zero story; the rule now carries trigger and
  action only.
- **Actor:** the owner.
- **Mechanism:** a rule in the local pack's RULES.md; nothing a file edit predicts brings a session
  to it, so it stays prose.

## 2026-09-25 · promoted · from a member's local pack (https://github.com/missingbulb/Claudinite/pull/2283)
