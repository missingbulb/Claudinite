...
description: Bootstrap Claudinite into a consuming repo — mount, hooks, checks, skills. Use when asked to bootstrap, adopt, or set up Claudinite, or to re-vendor a repo's mount to pick up updates.
metadata:
  body: workflow
  usage:
    expect: adoption
...
to a repo outside the session's own scope 403s with `Use add_repo to request access` — which the
repo being adopted usually is, so a shell recipe fails exactly where this is most needed):
