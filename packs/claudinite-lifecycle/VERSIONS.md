...
| 60824.1 | 2026-08-24 | The `scheduler-workflow-shape` entry-point rule accepts the queue's new home (#1317): `packs/claudinite-tasks/queue/(scheduler-run|tick).mjs` beside the pre-move `engine/scheduler` spellings, which pass until each member's workflow PR lands.The update worker lands its PR through `claudinite-tasks/shared-code/delivery.mjs`. |
