...
test("a marker naming a file in the person's own pack is clean", () => {
  const files = [
    ...
    'preferences/acme-user/provenance/turn-callout.md',
    'preferences/acme-user/provenance/saying-lgtm.md',
  ];
  const texts = { 'preferences/acme-user/RULES.md': PREFS.replace('at hand.', 'at hand. (saying-lgtm)') };
...
test("only the pack's own RULES.md is read, not every Markdown file in it", () => {
  ...
  const files = ['preferences/acme-user/skills/s/SKILL.md', 'preferences/acme-user/provenance/turn-callout.md'];
...
test('inert where this repo is not the store, where the pack names none, and for files store-file-names already reports', () => {
  ...
  assert.deepEqual(rule.run(ctx(['preferences/acme-user/RULES.md'], { 'preferences/acme-user/RULES.md': PREFS }, null)), []);
