...
test('a resolvable store is the healthy case and says nothing', () => {
  assert.deepEqual(runRule(rule, ctx({ 'claude-code-web-users-support': { repo: 'owner/name' } })), []);
  assert.deepEqual(runRule(rule, ctx({ 'claude-code-web-users-support': { repo: 'o/n', path: 'team' } })), []);
});

test('a declared pack with no store is reported, with the fix in the finding', () => {
  const [f] = runRule(rule, ctx({}));
...
test('a store that does not resolve is reported as such — not as "none"', () => {
  ...
  const [f] = runRule(rule, ctx({ 'claude-code-web-users-support': { repo: 'ownername' } }));
