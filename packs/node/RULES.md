...
- **A throwaway script that imports a project dependency can't live in an external scratchpad.** Node's module resolution walks up from the *script's own directory* looking for `node_modules`, which never reaches the project's tree when the script sits outside it — the import fails with `ERR_MODULE_NOT_FOUND` even though the dependency is installed, and `NODE_PATH` doesn't fix it (it has no effect on ESM resolution). A scratch script that needs a project dependency has to live inside the project (a gitignored scratch directory works fine), not in the harness's separate scratchpad.
