...
|---|---|---|
| 60902.3 | 2026-09-03 | `promote-to-canon` adds three items: a throwaway script needing a real project dependency can't live in an external scratchpad — Node's resolution walks up from the script's own location and never reaches the repo's `node_modules`; `btoa`/`atob` operate on Latin1 code units and throw or corrupt non-Latin1 text; check what Node version CI actually pins before relying on a version-gated runtime feature (#1524). |
