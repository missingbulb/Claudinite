...
Records for `packs/web-speech/pack.mjs`'s `version` field, one row per bump — added going forward from
...
| 60902.1 | 2026-09-02 | Names the OS-rendered-TTS self-echo case explicitly: the browser's own echo cancellation only attenuates audio the browser itself played, so a TTS path outside `speechSynthesis` needs an application-level echo guard (#1508). |
