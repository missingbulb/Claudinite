...
// Prose only — these are runtime browser behaviours, not repo-state signatures a
// static check could test. Fingerprinted by an actual speech-API reference in
// JS/TS source (the marker only *suspects* the pack; declaring it is the project's
// call, like every pack).
const SPEECH_API =
