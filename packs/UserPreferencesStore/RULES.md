# UserPreferencesStore — this project's people, in their own words
...
subject and load as prose. What this pack carries is *personal*: the same rules travel
with a person across every project they work in, and two people on one project can want
...
**The pack holds an address, not the content.** Preferences belong to a group of
people rather than to any one repository, so this project's declaration names the
**store** — a repo, and a path inside it holding one `<email>.md` per person:
...
**Every miss is fail-soft.** No identity, no configured store, no file for this person,
a fetch that fails — each is one plain-text note and the session proceeds on default
interaction behavior. Nothing here is load-bearing, and a nice-to-have must never be
