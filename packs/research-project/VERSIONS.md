...
Records for `packs/research-project/pack.mjs`'s `version` field, one row per bump — added going forward from
...
| 60903.1 | 2026-09-03 | The activity-bound procedure moves out of `RULES.md` into six skills — `ground-truth-extraction`, `source-intake`, `add-an-external-corpus`, `write-an-iteration-note`, `define-and-calibrate-metrics`, `mine-session-for-preferences` — and `RULES.md` shrinks to the always-on rules (#1662). |
