...
> This is the **unattended daily** routine. **Unlike** the owner's on-demand, in-session "learned lessons" command (which delivers a PR for review), this routine commits straight to `main`: it writes only the project's *own* local packs, and the owner has opted out of a per-run review gate here to keep the fleet's daily lesson-capture from piling up PRs. The shared canon stays gated — lifting anything up into it is [promote](../canon-curation/promote.md)'s job, and promote *does* open a PR.
