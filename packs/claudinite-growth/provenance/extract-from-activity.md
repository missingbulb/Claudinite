...
- **Reason:** the two extraction halves fired at one anchor over one corpus, so the method moved
  into skills the task loads in order and a second caller needs no copy.
- **Actor:** @missingbulb (owner).
- **Model:** Claude, per the commit trailer.
- **Mechanism:** the extract-from-activity skill, body workflow, reached by its description.
