...
|---|---|---|
| 60827.1 | 2026-08-27 | The capture ladder gains a rung 0 — a comment at the call site — and `growth-write-scope` admits a comment-only edit anywhere in the repo, granted per file by stripping comments and comparing rather than by the run asserting it. `extract-from-activity` gains a comment pass over the window's touched files. A new check declares `since` and lands at its real severity even where the tree still violates it, so "the tree isn't clean yet" stops forcing a lesson down to prose; prose is rationed at two one-sentence rules per run (#1378). |
