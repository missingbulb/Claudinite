...
|---|---|---|
| 60822.1 | — | `usage-fold` folds daily rather than hourly, and its signal window moves with it — a frequency finer than the cron's two ticks a day cannot be honoured, and a window still sized to an hour would never see a captured session again (§17.1). `growth-extract`'s anchor offset retires; the `after:` it already declared is what orders it. |
