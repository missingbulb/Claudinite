...
3. **Open a PR** — one PR for the run's conversions on a per-run-unique branch, **titled `Claudinite growth: prose to checks`**, its commit referencing the tracking issue so the `task-lifecycle` gate passes. Keep the repo's offline test suite green before opening.

   The title is not cosmetic: this task's own precondition keys on it, and only that subject stops next week's run opening a second sweep on top of this one while it waits for review. A round titled anything else is invisible to the gate.
