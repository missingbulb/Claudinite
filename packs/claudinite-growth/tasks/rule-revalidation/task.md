...
4. **Open a PR** — one per run, on a per-run-unique branch, **titled `Claudinite growth: rule revalidation`**, its commit referencing the tracking issue so the `task-lifecycle` gate passes. The PR body is the evidence a reviewer cannot re-derive: per changed rule, the probe run and its result. Keep the repo's offline test suite green.

   The title is not cosmetic: this task's own precondition keys on it, and only that subject stops next week's run re-probing claims whose corrections are already in this PR awaiting review. A round titled anything else is invisible to the gate.
