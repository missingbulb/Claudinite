...
Open a PR and leave it for review. This task rewrites the rules sessions obey, on evidence a reviewer cannot re-derive from the diff, so every run's changes go in front of the owner. A later round's evidence replaces an earlier round's still waiting: the pull request an earlier run left open closes as superseded once yours exists, which your converge performs when handed `--pr`.
