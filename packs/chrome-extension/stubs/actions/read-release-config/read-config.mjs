...
export const CONFIG_PATH = '.github/release.config';
...
// valid, absent is not.
//
// PURE, and returning its complaints rather than exiting on them, because this
// action is not the only reader: the pack's `cer/version-bumped` check imports
// it to resolve the repo's own ship_paths, and one parse of the release config
// is what keeps the check's idea of what ships identical to the pipeline's.
// The CLI below is what turns an error into a failed run.
export function parseConfig(text) {
  ...
  text.split('\n').forEach((raw, i) => {
    ...
    if (eq === -1) {
      errors.push(`${CONFIG_PATH}:${i + 1} is not KEY=value or a # comment: "${line}"`);
      return;
    }
  ...
  if (missing.length) errors.push(`${CONFIG_PATH} is missing required key(s): ${missing.join(', ')}`);
  const unknown = Object.keys(cfg).filter((k) => !REQUIRED_KEYS.includes(k));
  if (unknown.length) errors.push(`${CONFIG_PATH} has unknown key(s): ${unknown.join(', ')} (valid: ${REQUIRED_KEYS.join(', ')})`);

  return { cfg, errors };
}

// The composite action's step. Everything it does is inside it, so importing
