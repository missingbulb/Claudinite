...
| 60903.6 | 2026-09-03 | `RULES.md` shrinks to the always-on rules; the sign-in and token rules become the `extension-google-signin` skill, ES modules in content scripts and the `web_accessible_resources` list `content-script-modules`, runtime host access `extension-host-permissions` (both forced for any `manifest.json` edit), and the CDP worker-probing rules `probe-extension-worker-over-cdp` (#1662). |
