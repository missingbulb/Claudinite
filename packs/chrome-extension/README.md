...
Active when a `manifest.json` declares `manifest_version` — the MV3 build/runtime gotchas that apply while you're *coding* an extension. Mostly prose (`RULES.md`), since most MV3 gotchas are runtime behavior with no signature in the artifact; the gotchas that do leave a signature are carried as checks.
...
## Gotchas (`RULES.md` + checks)

| Rule (≤5 words) | How enforced |
|---|---|
| MV3 worker paths must be root-absolute | prose |
| SetIcon needs imageData, not path | `chrome-extension/set-icon-image-data` (blocking) |
