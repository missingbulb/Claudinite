...
**Six tasks** — separate, because they close on unrelated conditions. The **digest**
([fleet-digest](tasks/fleet-digest/task.md)) writes the owner's morning brief — the only output
addressed to a person rather than to the machinery, and the only task whose agent stage runs here.
The **roster sweep**
...
**Writing anything here that ranks or reports fleet activity** — filter the machine's own
artifacts out of **every** stream you collect, not just the first one you thought of. Claudinite is
the busiest actor in this fleet, and its bookkeeping does not merely appear in a size-or-discussion
ranking, it wins it: a dispatch issue collects a comment per executor stage, so it outweighs the
work it dispatched. Recognize a machine artifact with the engine's own predicate —
`isDispatchTitle` from [dispatch.mjs](../../engine/scheduler/dispatch.mjs) — rather than a private
regex, since the
dispatch-title format is the scheduler's to change. Filter where the maintenance total is
**tallied**, never inside the fetch: an issue the machinery filed and closed is still a true account
...
choice. `digest` (default: everything) is the brief's two knobs, `pick` and `nudge`
([digest-config.mjs](tasks/fleet-digest/digest-config.mjs) spells out what each accepts). Every key
defaults, so an existing sheepdog config keeps working untouched.
...
*member*, run by that member's own executor under its own grant. The digest's agent crosses no
boundary — it reads what prework already fetched and writes one file here — which is why it may
run at all. That is not an implementation detail but the trust model
...
only a repo edit can finish is the member's own adopt-requested-packs task's, ceilinged at
`open-pr` *there*, because declaring a pack switches on checks that run in the member's CI the moment they land, so it is
...
`daily+1h` so it reads a fleet the other sweeps have finished with. There is **no coverage
