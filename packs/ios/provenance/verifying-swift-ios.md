## 2026-09-06 · born · Promote the reviewed survivors of nine growth-promote PRs (#1671)
- **Source:** the growth-promote run of 2026-08-22 over the members whose local packs moved in that
  window, missingbulb/ClaudiniteWebsite, EdFringeNow, TLDR and WIP; the run's own pull request
  (#1204) was closed in favour of this consolidation, which is where the text landed. The evidence
  names the members as a set rather than one per rule.
- **Reason:** both rules were weighed against the four-moment check vocabulary in the same review
  and neither carries a signature it can express, so both stay prose.
- **Actor:** the growth-promote run, reviewed and merged by @missingbulb (owner).
- **Model:** Claude Fable 5.1, per the commit trailer.
- **Mechanism:** a RULES.md rule. The run proposed it headline-first, as an assertion that every
  Swift/iOS change is verified by CI alone; the review re-keyed it to the moment it applies and cut
  it to the prose ration, 73 words to 51.
- **Landed:** #1671 (Refs #1202, #1308, #1408, #1435, #1657, #1672) · pack version 60906.1.
