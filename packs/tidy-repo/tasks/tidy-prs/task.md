...
actually moved in the window — then record the picture. You run under the executor, dispatched by a
`ready-for-agent` issue; its **Context section is binding scope** — it lists the PRs to assess. Work only
...
title, never a fuzzy match**; create it **already closed** if absent — creation always lands an issue open and
ignores a `state: closed` argument, so create it and close it in a second call (never a fresh issue per run,
never a bare number that can dangle). Each dimension keeps its **own** tracker, so three tasks never race to
rewrite one body.
