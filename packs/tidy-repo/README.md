...
The repo tidy-up as a composable pack: the nightly PR/issue sweep, contributed to the fleet
...
work is a maintenance task, not checks. Its policy (`RULES.md`): assess PRs read-only, act only on
issues.
...
|---|---|---|---|
| PRs — assess only. | critical | correctness | prose: 32 words |
| Issues — act. | medium | correctness | prose: 84 words |
| Trackers — record changes, not scans. | low | complexity | prose: 117 words |
...
Each applies its per-object skill (`single-issue-triage` / `single-pr-status`) across the targets the
precondition hands it, then rewrites its tracker (`Claudinite tracker: Tidy Issues` / `Tidy PRs`) from
...
touched, no `tidy-issues`; no open PR opened or updated, no `tidy-prs`. The verdicts over an unmoved
