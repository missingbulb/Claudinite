...
repo it is true most days), a PR that merged, and a push to the default or infra branches. Nor
can `tidy-issues` be its own movement: `single-issue-triage` posts nothing when the verdict is the one it
already posted there, so a standing verdict never re-arms tomorrow's run.
