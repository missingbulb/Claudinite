...
- **Issues — act.** Take the first applicable action: close-if-implemented / needs-decision / blocked
  ...
  defaults to the reversible option (comment / leave) when the check is inconclusive. A verdict identical
  to the one already posted on that issue is **not** an action — say nothing, or the comment becomes
  tomorrow's trigger.
