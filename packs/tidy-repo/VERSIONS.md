...
|---|---|---|
| 60830.3 | 2026-08-30 | `improve-comments` declares its safety case as policy — `may_automerge: ['comment-only-changes', 'readme-changes']` — so the landing verdict is measured by the policy engine, with the skill's own scope gate still redding the same boundary on every branch wearing its title (#1459). |
