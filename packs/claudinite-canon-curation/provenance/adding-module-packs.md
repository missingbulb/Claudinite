...
- **Mechanism:** a rule in the `claudinite` local pack's RULES.md, beside the check that enforces
  its await half.
- **Landed:** #606 (Refs #599).
...
- **Mechanism:** the rule moves from the `claudinite` local pack's RULES.md into this pack's first
  RULES.md. Its clause naming the pack's own check comes out, which the narration rule forbids in
  prose.
