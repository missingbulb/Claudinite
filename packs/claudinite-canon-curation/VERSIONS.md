...
|---|---|---|
| 60831.1 | 2026-08-31 | First canon version. Promoted from the canon home's `canon-curation` local pack and generalized over any canon: the write surface is the `packs/` shelf plus whatever `write_paths` declares, the skill check is anchored on a pack tree instead of the engine's own tracked registry, and the prose names a canon rather than this one (#1537). |
