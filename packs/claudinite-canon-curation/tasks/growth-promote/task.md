...
- **Never bump a pack's `version` or write a `VERSIONS.md` row** — both are `pack-version-bump`'s to write once this change has landed, and it is the only writer of either. A bump here also puts a code change into an otherwise prose-only diff, which is what holds the delivered pull request back from this task's automerge policy.
