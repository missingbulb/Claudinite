...
The [growth lifecycle](../../README.md)'s central stage: read the target members' local packs, lift the portable lessons into the shared canon, and open a PR against the canon's default branch **for the owner to approve**. Declared only by the canon's home repo, so it runs **once, centrally**, never per project.
