...
- **Carry the lesson's provenance.** Read the element's file in the member's local pack (`provenance/<slug>.md`, named by the marker that ends the rule), reduce it for the canon's boundary - `provenance.mjs reduce <file>` (`--public` where this canon is public): handles to roles, session ids and quotes dropped, member locators generalized - and land the reduced text as the canon element's file, ending the promoted rule with its marker (a slug that collides on the shelf takes a suffix, and the marker follows it), then append `promoted · from a member's local pack (<this PR>)`. The member's file is never edited. A lesson landed as a check takes the check's file the same way.
...
- **Never bump a pack's `version` or write a `VERSIONS.md` row** — both are `pack-version-bump`'s to write once this change has landed, and it is the only writer of either. A bump here puts a code change into an otherwise prose-only diff, and strands whichever members took the first of two changes claiming one number.
