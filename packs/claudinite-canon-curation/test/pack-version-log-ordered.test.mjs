...
test('rows already descending newest-first are clean', () => {
  assert.deepEqual(runRule(rule, ctx({
    'packs/demo/provenance/VERSIONS.md': log(['60830.2', 'Second.'], ['60830.1', 'First.']),
  })), []);
});
...
test('a row sitting below an older one is the violation — the tail must not read newer than what precedes it', () => {
  const findings = runRule(rule, ctx({
    'packs/demo/provenance/VERSIONS.md': log(['60824.1', 'Oldest, at the top by mistake.'], ['60830.5', 'Actually newest.']),
  }));
...
test('each pack is judged on its own — one pack\'s order says nothing about another\'s', () => {
  assert.deepEqual(runRule(rule, ctx({
    'packs/demo/provenance/VERSIONS.md': log(['60830.2', 'Second.'], ['60830.1', 'First.']),
    'packs/other/provenance/VERSIONS.md': log(['60820.1', 'Only.']),
  })), []);
});

test('a log deeper than a pack root is not a pack\'s log', () => {
  assert.deepEqual(runRule(rule, ctx({
    'packs/demo/skills/x/provenance/VERSIONS.md': log(['60820.1', 'Oldest.'], ['60830.1', 'Newest.']),
  })), []);
});

test('a row whose first cell is not a version is skipped, not compared', () => {
  assert.deepEqual(runRule(rule, ctx({
    'packs/demo/provenance/VERSIONS.md': '| 60830.0 | — | Not a version. |\n| 60830.0 | — | Nor this. |\n',
  })), []);
});
...
test('the canon\'s own packs run every version log newest-first', () => {
  const tracked = execFileSync('git', ['ls-files', ':(glob)packs/*/provenance/VERSIONS.md'], { cwd: REPO, encoding: 'utf8' })
    .split('\n').filter(Boolean);
  ...
  assert.deepEqual(runRule(rule, ctx(files)).map((f) => `${f.file}: ${f.what}`), []);
});
