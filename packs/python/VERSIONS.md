...
| 60903.2 | 2026-09-03 | The extra-declaration and `# noqa: F401` probe rules move out of `RULES.md` into the `python-optional-deps` skill, now forced for `pyproject.toml`, `setup.cfg` and `setup.py` (root or one directory down); the prose shrinks to the always-on stdlib-backend rule (#1662). |
