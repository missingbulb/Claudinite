...
- **Reason:** decoration often buys the coupling the rule exists to prevent for nothing, so a waiver
  is for a crossing that genuinely must exist.
...
- **Actor:** @missingbulb (owner).
- **Mechanism:** a RULES.md rule, triggered on "Waiving a finding on text rather than code".
