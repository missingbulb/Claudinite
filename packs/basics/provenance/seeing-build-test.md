## 2026-06-20 · born · Add portable Claude rules docs (5235b9d3)
- **Source:** `working-discipline.md`.
- **Reason:** a clean run is what makes a genuinely new warning stand out, so noise here costs
  detection later.
- **Actor:** @missingbulb (owner).
- **Mechanism:** prose.
- **Landed:** commit 5235b9d3.

## 2026-08-12 · reworded · Rewrite RULES.md as situation-keyed rules, and write down the method (#760)
- **Actor:** @missingbulb (owner).
- **Mechanism:** a RULES.md rule, triggered on "Seeing a build, test or CI warning".
- **Landed:** #760, closing #759 · pack version 1.
