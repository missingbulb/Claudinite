...
test('skill-description-length: flags a description past 30 words, and reads no other line in the file', () => {
  const root = makeRepo({ changed: {
    'packs/acme-pack/skills/acme-skill/SKILL.md': skillFile('acme-skill', 31, 10),
    ...
    'packs/acme-pack/skills/acme-other/SKILL.md': skillFile('acme-other', 20, 400),
  } });
  try {
...
// 30 and 31 are the only two inputs that tell `>30` apart from `>=30`, so they are the
// cases; sampling either side comfortably agrees with both and pins neither.
test('skill-description-length: 30 words is inside the cap, 31 is over it', () => {
  const root = makeRepo({ changed: {
    'packs/acme-pack/skills/acme-at-cap/SKILL.md': skillFile('acme-at-cap', 30, 5),
  } });
  try {
