...
- When a practice is withdrawn, rewrite the docs that taught it into the **current** rule in
  the present tense — never leave a passage narrating what used to be done and why it stopped.
  A reader who never knew the practice does not need to be taught it in order to be told the
  rule, and describing a withdrawn technique is how it keeps getting rediscovered; the history
  belongs to the commit that removed it. This bans narration of a *superseded* way of working,
  not evidence: measured costs that argue a live rule is worth its space stay. And retiring a
  practice is not the moment to install a guard against it — an automated check earns its place
  from a mistake that actually repeats, not from the deletion that prompted it.
