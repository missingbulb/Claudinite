...
|---|---|---|
| 60830.2 | 2026-08-30 | Rule revalidation (#1428): `ToolSearch`'s bare-keyword search now resolves a real tool (only a genuinely-absent name returns "no matching tools"), and `Edit` no longer requires a prior Read-tool call — the harness tracks a file's content from any write or shell inspection already made this session. |
