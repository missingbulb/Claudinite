...
Before building a mechanism for a behavior, verify against a real run that it isn't already provided — and before hand-authoring release, deploy, versioning or CI plumbing, look for the shared pack that already owns it. A mechanic you are about to **copy from a sibling repo** is the tell: the second copy is a candidate to raise centrally, not a file to author locally, because a bespoke third copy gets deleted later and takes its reviewed workflow and CI entry with it. If no pack owns it and a sibling already solved it, that is the finding to report — not the code to duplicate.

Never publish an unverified fact about a real, named person. When the primary source is blocked, do **not** substitute a data broker or aggregator, and do **not** ship the claim with a caveat for the owner to sanity-check later — ask. Flagging is not verification here: the cost is not a wrong paragraph but a public claim about a named individual that they never made, and the person is usually one question away.
