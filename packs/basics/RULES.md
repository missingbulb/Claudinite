...
Never publish an unverified claim about a real, named person — their employers, their qualifications, their history — onto any artifact that ships. When the primary source is unreachable, do **not** substitute a data aggregator and do **not** ship the claim with a "please sanity-check this" caveat: flagging is not verification, and the cost of being wrong is a public statement someone never made. Ask the person, or the owner who can reach them; one question is cheaper than any research.

Three harness-tool contracts worth knowing before you spend a call rediscovering them:

- **A keyword `ToolSearch` that finds nothing is evidence about your query, not about the environment** — the deferred tools are matched on keywords that aren't the tool's own vocabulary, so a capability can sit in the session while the search denies it. Before telling the owner a step is theirs because the tool is missing, search the **literal, fully-qualified name** (`select:mcp__<server>__<tool>` — the short name alone returns "no matching deferred tools", which reads exactly like absence) and then try the tool. Copy the name off the deferred-tools list rather than typing it from memory.
- **`Edit` requires that the file was read *with the read tool*** — inspecting it with `cat`/`grep`/`sed` does not count, however much of it is already on screen. The trap is the natural rhythm of surveying an unfamiliar tree in the shell and then editing what you found: the moment shell output tells you which file you are about to change, read that exact path (a narrow offset/limit window is enough — the precondition is the call, not the byte count).
- **Don't fetch content you need verbatim with a summarizing fetch tool.** Asked for a file's exact text it will return a description of it, and ordinary publisher pages routinely answer `403`. When the bytes matter, `curl` into the scratchpad and read from disk. On a `403`, don't retry it or try a sibling URL: take the search snippet, attribute it to the publisher rather than asserting it, and mark it for re-verification.
