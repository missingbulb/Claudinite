...
A session that starts with no pack prose and no owner preferences in context — only the project's instruction file — and gets no check feedback between turns, is a session whose SessionStart hook did not fire. Treat that combination as the tell, not as "this project has no rules": re-run the session-start hook command before doing the work, because the rules you'd otherwise proceed without include the ones that decide how the change lands.
