...
Before working around a finding from a *vendored* check — accepting it, suppressing it, or concluding an upstream fix hasn't landed yet — confirm the vendored copy is current. A vendored engine reflects the stamp of its last refresh, not upstream's default branch, so the finding you're about to paper over may already be fixed upstream and simply not pulled in. Read the stamp, and check whether a refresh has run since the upstream fix merged; a workaround committed against a stale mount is one someone has to find and remove later, and it carries a reason that was never true.
