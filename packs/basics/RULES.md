...
- **Classified a feature** — agree on the requirement, record it in the project's requirements
...
- **Classified a process change** — the owner is changing *how* work is done. Land it as durable
...
- **A warning you can't fix now with a small cause-addressing change** (it waits on an upstream
  ...
  open, then move on. Resolving it, by real fix or a consciously-chosen suppression, happens in that
  issue's own change.

- **An approval — to merge, to ship, to proceed** — it applies only *backward*, to the work
...
- **A chosen answer to a multiple-choice prompt** — it isn't authorization, even when an option's
  wording mentioned the action.
