## 2026-07-07 · born · github-actions: check that a Pages artifact prunes tooling symlinks (#158)
- **Source:** an EdFringeNow session: the member's live-site Pages deploy broke from the moment
  Claudinite was bootstrapped and stayed red for days, until an unrelated merge resurfaced it.
- **Reason:** bootstrap commits `.claude/skills/*` as symlinks into a gitignored corpus that is
  empty in CI, and `actions/upload-pages-artifact` tars with `--dereference`, so the dangling links
  fail the upload and silently block every Pages deploy.
- **Actor:** @missingbulb (owner).
- **Model:** Claude Opus 4.8, per the commit trailer.
- **Mechanism:** check gha/pages-artifact-symlinks, blocking, firing only where the repo mounts
  skills as symlinks and a workflow uploads the repo root; silent once a prune step runs before the
  upload or a dedicated build directory is uploaded instead.
- **Rejected:** a prose note. The ladder puts a check above a doc note, and the check fails if the
  prune is missed, so there was nothing left to document.
- **Landed:** #158 (Refs #157).
