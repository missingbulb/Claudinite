...
- **Unrelated concerns in one PR get one commit each** — a commit is the unit of *one* task, so two fixes that could each stand alone (and could each be reverted, cherry-picked, or bisected to on their own) never share one. This is the rule the two above are instances of: they layer a *single* task's work, and this one keeps *separate* tasks from ever being layered together in the first place. A PR that legitimately carries several — a sweep that turns up three independent defects, a batch of small corrections — is fine; folding them into one commit is what isn't, because it makes the diff unreadable by concern and welds fixes together that have nothing to do with each other. Write each commit's message about its own concern alone: if you find yourself numbering unrelated items inside one message, that message is describing the split you should have made. When the mixing is only noticed after committing, `git reset --soft <base>` and re-stage by concern (`git add` per path) rather than leaving it.
