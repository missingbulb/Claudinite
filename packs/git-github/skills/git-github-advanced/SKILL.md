...
A `push` or `workflow_dispatch` run isn't attached to a PR, so the PR-scoped check-run query above doesn't apply to it. Confirm such a run through the GitHub API/MCP tools: `get_job_logs(run_id, failed_only: true)` — "0 failed jobs" means green — or, for a release build, `get_release_by_tag`. `get_job_logs` needs more than a bare `run_id`: it rejects with "job_id is required when failed_only is false" unless you pass `failed_only: true` or fetch a `job_id` first (`actions_list`'s `list_workflow_jobs` method — nothing answers to that bare name), and it 404s for a job still `in_progress` — wait for the job to finish. A `curl` of the run's status does reach GitHub: the sandbox's egress proxy injects the session's own credentials, so a repo-scoped `api.github.com` GET answers `200` — even `/user` resolves with no `Authorization` header. What it won't do is fail loudly, so a `curl`/`Monitor` loop that derives a pending-*count* without checking the fields it read reports "all checks concluded" within a second on any body it doesn't recognise. Verify any curl-based result against `pull_request_read`'s `get_check_runs` method before acting on it.
