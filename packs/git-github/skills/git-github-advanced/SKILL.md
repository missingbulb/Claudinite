...
## A step only a human can perform goes in its own issue, never a note in the PR body

When a change lands something that cannot work until a person flips a setting the repo can't set for itself (a Pages source, workflow permissions, a repository variable, a deployment-environment rule), open an **issue** for it — one checkbox per setting, each saying what stays broken while it is off, plus a stated closing condition — and link it from the PR. A merged PR's description is not a to-do list anyone returns to: the note merges and disappears with it, and the setting gets re-discovered days later. Keep the note in the PR as well if you like; the issue is the part that survives the merge. The one exception is a step whose home is an artifact the owner is already editing. And before handing a step over at all, confirm you genuinely cannot do it yourself.
