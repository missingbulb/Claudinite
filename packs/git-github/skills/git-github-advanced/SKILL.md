...
## Auto-merge waits on what a branch REQUIRES, not on what its CI runs

`enablePullRequestAutoMerge` is a queue for a PR that *cannot* merge yet. Arming one that already can is rejected outright — `Pull request is in clean status` — so on a base branch with no protection and no ruleset (`"protected": false`), auto-merge is unavailable no matter how much CI the repo runs: the checks report, nothing requires them, and the PR is mergeable the whole time. Enabling the repo's *Allow auto-merge* setting does not change this; the setting permits the feature, a required check is what gives it something to wait for.

So a green suite is not evidence that auto-merge will work, and **"has PR CI" is the wrong predicate to gate automation on** — ask what the base branch requires (`GET /repos/{repo}/branches/{base}` for `protected`, `GET /repos/{repo}/rules/branches/{base}` for rulesets; the `/protection` endpoint wants admin the `GITHUB_TOKEN` lacks). Where nothing is required, an automation that means "land this once it's green" has to wait for the runs and merge itself. Two failure modes if you skip the distinction: the arm fails every cycle and the PR sits open, and — worse — the error text sends the owner to fix a setting that was never the cause.
