...
- **When a Stop-hook or CI finding names its own fix as a plain amend, take that fix — don't reach for history-rewriting machinery instead.** A check whose failure message already says "amend the latest commit" is answered by exactly that; scripting a `filter-branch`/rewrite pass across every prior commit to backdate the fix into each one is needless extra risk (a wrong regex, a bad force-push) for no benefit over the one-line remedy the finding already gave you.
...
A `push` or `workflow_dispatch` run isn't attached to a PR, so the PR-scoped check-run query above doesn't apply to it. Confirm such a run through the GitHub API/MCP tools: `get_job_logs(run_id, failed_only: true)` — "0 failed jobs" means green — or, for a release build, `get_release_by_tag`. `get_job_logs` needs more than a bare `run_id`: it rejects with "job_id is required when failed_only is false" unless you pass `failed_only: true` or fetch a `job_id` first (`list_workflow_jobs`), and it 404s for a job still `in_progress` — wait for the job to finish. Don't `curl` the run's status instead: in a sandboxed session `api.github.com` is proxy-blocked and returns an error body that never matches a success pattern, so a `curl`/`Monitor` poll silently reports "still running" until it times out. The same proxy-blocked body can also fail the *other* way: a loop that derives a pending-*count* from the response body can read an error object's absent fields as zero pending and report "all checks concluded" within a second of starting, while the real checks are still `in_progress`. Verify any curl-based result against the GitHub MCP tools (`get_check_runs`) before acting on it, rather than trusting either silence to mean what it looks like.

## A CI artifact behind a storage-provider URL is often unreachable from a sandboxed session — reproduce locally instead

`download_workflow_run_artifact` hands back a blob-storage URL (e.g. `*.blob.core.windows.net`), and a sandboxed session's egress proxy commonly denies it at CONNECT — chasing that link burns a call and a probe for nothing. Use `get_job_logs` with a large `tail_lines` to learn *which* case or step failed, then reproduce and fix it locally rather than trying to pull the artifact down.

## A long-running workflow that commits generated files will eventually race a more-frequent scheduled writer

A workflow that regenerates and commits derived/generated files and can run for longer than a competing scheduled job's own interval is not occasionally rejected on push — it's *guaranteed* to be, once the scheduled job lands a commit first. Recovery has to **re-derive, not merge**: the contested files are generator output from both writers, so a rebase would conflict on generated bytes whose only correct value is "whatever the generator computes now," not either side's stored version. On a rejected push, fetch the current base, regenerate from that base plus this run's own non-generated input, and retry — design the commit step for this from the start rather than discovering it after a long run's work is lost.
...
## Write access scoped to one repo doesn't license routing around the boundary for a fix that belongs elsewhere

A session's push/write access is commonly scoped to a single repo, even when it can *read* others (a cloned dependency, a monorepo neighbour, an upstream project). Finding a genuine bug in a repo you can only read is not license to hunt for a way to push there anyway — that scoping is a deliberate boundary, not an oversight to route around, and it doesn't move for a good reason. Finish and verify the fix in a scratch clone of the repo that owns it, then attach it as a diff/patch to an issue in the repo you *can* write to, carrying what you verified — so a session with the right scope can `git apply` it and land it properly reviewed.
