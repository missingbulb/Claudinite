...
## `git checkout <branch> -- <paths>` never carries uncommitted edits to a new branch

`git checkout <other-branch> -- <paths>` restores those paths' **committed** content over the working tree, so reaching for it to bring in-progress edits along to a fresh branch silently destroys exactly the edits it was meant to preserve — no conflict, no warning, and nothing to recover from (the edits were never in the object store). `git checkout -b <new> origin/main` already carries a clean working tree across; when it can't, `git stash` (or commit) first — and never inside the same `&&` chain that moves the branch, where the destructive half runs before you can look at the result. Verification the edits enabled dies with them: re-run the suite after any such recovery rather than trusting a green run from before.
