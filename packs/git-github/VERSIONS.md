...
|---|---|---|
| 60903.2 | 2026-09-03 | `git-github-advanced` gains the write-scope-boundary pattern from `promote-to-canon` #1202: package a fix that belongs in a repo outside the session's write scope as a patch on an issue in a repo you can write to; prefer an amend over `filter-branch`; a `Monitor`/`curl` false positive reading `api.github.com`'s error body as "zero pending"; a CI-artifact-blocked-in-sandbox reproduction path via `get_job_logs`; the long-running-workflow-races-a-scheduled-writer recovery (#1204). |
