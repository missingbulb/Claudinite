...
| 60831.1 | 2026-08-31 | `gha/scheduled-failure-escalation` widens to `workflow_run`/`repository_dispatch` triggers, not only `schedule` — a workflow chained off another run or fired by an external dispatch is just as unwatched. Five `git-github-advanced` additions: `list_pull_requests`'s unreliable `merged` field, `search_code`'s undercounting for exhaustive enumeration, subscribing every PR a batch leaves open (not a sample), amending rather than rewriting history to add a missed issue reference, and caching a repo's "no PR template" finding (#1508). |
...
| 60902.1 | 2026-09-02 | Two `git-github-advanced` additions: `actions_list`'s `list_workflow_runs` can ignore `per_page` entirely, so shrinking it doesn't shrink the response — go straight to a head-SHA/run-id-scoped query or the spilled overflow file; and `issue_read`'s `get_*` methods reject a PR number outright, where `pull_request_read` is what resolves it (#1584). |
