...
|---|---|---|
| 60903.2 | 2026-09-03 | `git-github-advanced` gains prose from MissingBulbWebsite and WIP: `codeload.github.com` can be blocked by a sandbox's egress proxy where an ordinary `git clone` isn't — use `git clone --depth 1` instead of a tarball fetch; resolve a wait on a GitHub Actions run/check through exactly one mechanism, never a background sleep alongside a separate blocking poll (#1345). |
