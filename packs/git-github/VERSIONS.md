...
| 60823.1 | 2026-08-23 | Promote-to-canon (#99): array-valued API fields (`labels`) are a full-replace on write; `head` needs an `owner:` prefix; a confirmed-templateless repo doesn't need re-probing for a PR template; the "already in clean status" auto-merge case now covers a path-scoped CI trigger that ran nothing. |
