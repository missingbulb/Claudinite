...
|---|---|---|
| 60904.1 | 2026-09-04 | `promote-to-canon` (#1671): `gha/scheduled-failure-escalation` also covers `workflow_run` and `repository_dispatch` triggers; `git-github-advanced` gains the `issue_write` labels-overwrite trap, amend-not-filter-branch, curl reading an error body as zero pending, one wait mechanism, a green run's skipped job, unreachable artifact URLs, generated-file races with a scheduled writer, `list_pull_requests`' `head`/`merged` caveats, `issue_read` on a PR number, subscribing every PR a sweep leaves open, `actions_list` ignoring `per_page`, `search_code` undercounting and `codeload.github.com` being blocked. |
