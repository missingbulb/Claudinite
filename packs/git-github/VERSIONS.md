...
|---|---|---|
| 60903.2 | 2026-09-03 | `git-github-advanced` gains prose from `promote-to-canon`: `list_pull_requests`'s `head` filter needs an `owner:` qualifier or it silently mismatches; Claude Code Web's PR-diff view can silently drop a root-level file addition — verify with `git ls-files`; wait on a GitHub Actions run or PR check through exactly one mechanism, never a background sleep alongside direct polling (#1372). |
