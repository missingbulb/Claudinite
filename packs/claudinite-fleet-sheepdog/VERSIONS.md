...
| 60903.2 | 2026-09-03 | The manual levers and the token rules move out of `RULES.md` into the `running-fleet-levers` skill, and the config rules (`exclude`, `packSeeds`, seeds-agree) into `configuring-the-fleet`, forced for `.claudinite-settings.json`; the prose shrinks to the always-on rules for acting on a sweep's report (#1662). |
