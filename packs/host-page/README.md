...
Declared by hand: it has no fingerprint, so a repo picks it up only by naming it in
...
All three are file-scoped or scoped to interfaces that model real user input, so a `CustomEvent`
you dispatch to your own listener is left alone. Everything else in `RULES.md` stays prose.
