...
| | Count |
|---|---|
| **Hardcoded conformance checks** | **39 canon** (11 basics + 1 barriers + 1 grow_with_claudinite + 9 github-actions + 8 chrome-extension-release + 3 aws-sam + 6 product-wiki) + the canon home's own [canon-curation local pack](../.claudinite/local/packs/canon-curation/README.md) (1 code check + the contributed `pack-independence` barrier); plus skill-owned checks (e.g. the `unattended-agents` skill's `routine-structure`, `in-session-github-access`, `task-declaration-shape`) that ride their pack's activation |
