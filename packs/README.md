...
|---|---|---|---|
| [basics](../packs-tests/basics/README.md) | declared (seeded by `--init`) | 11 | ~8 (working-discipline + task-lifecycle) |
| [barriers](../packs-tests/barriers/README.md) | declared (or pulled in via `requires`) | 1 | 0 (config-driven segregation) |
| [grow_with_claudinite](../packs-tests/grow_with_claudinite/README.md) | declared (seeded by `--init`, opt-out by removal) | 1 | growth member-side daily tasks (extract / dedup / pack discovery / conversation-extract) + in-session merge capture |
| [tidy-repo](../packs-tests/tidy-repo/README.md) | declared (seeded by `--init`, opt-out by removal) | 0 | policy (assess-only-vs-act) + tidy daily tasks |
| [sheepdog](../packs-tests/sheepdog/README.md) | declared (opt-in; the fleet-enforcer repo only) | 0 | fleet-enforcer marker + config + coverage workflow stub |
| [github-actions](../packs-tests/github-actions/README.md) | `.github/workflows/` | 8 | 0 |
| [chrome-extension](../packs-tests/chrome-extension/README.md) | manifest_version manifest | 0 | 8 |
| [chrome-extension-release](../packs-tests/chrome-extension-release/README.md) | single `Release to Chrome Store` stub (opt-in) | 8 | 0 (+ RELEASE contract) |
| [node](../packs-tests/node/README.md) | root package.json | 0 | 2 |
| [python](../packs-tests/python/README.md) | pyproject.toml near root | 0 (2 in its skill) | 3 |
| [web-speech](../packs-tests/web-speech/README.md) | speech API in JS/TS source | 0 (2 in its skill) | 15 |
| [aws-sam](../packs-tests/aws-sam/README.md) | SAM template | 3 | 3 |
| [google-identity](../packs-tests/google-identity/README.md) | declared | 0 (3 in its skill) | 0 |
| [html](../packs-tests/html/README.md) | declared | 0 | 1 |
| [flutter](../packs-tests/flutter/README.md) | pubspec.yaml | 0 | 0 (stub) |
| [firebase](../packs-tests/firebase/README.md) | `firebase.json` | 0 | rules / functions / deploy discipline |
| [firebase-release](../packs-tests/firebase-release/README.md) | declared (opt-in) | 0 | dev/prod split + App Check store gating |
| [android](../packs-tests/android/README.md) | `AndroidManifest.xml` | 0 | stub |
| [ios](../packs-tests/ios/README.md) | `ios/Runner/Info.plist` | 0 | stub |
| [play-store-release](../packs-tests/play-store-release/README.md) | declared (opt-in) | 0 | stub |
| [app-store-release](../packs-tests/app-store-release/README.md) | declared (opt-in) | 0 | stub |
| [research-project](../packs-tests/research-project/README.md) | declared (class) | 0 | 54 (14 sections) |
| [product-wiki](../packs-tests/product-wiki/README.md) | declared (marker: `product-wiki/product-requirements/README.md`) | 6 | wiki growth discipline + weekly growth daily task |
| [spec-driven-product](../packs-tests/spec-driven-product/README.md) | declared (class) | 0 | 25 (8 sections) |
| [executable-requirements](../packs-tests/executable-requirements/README.md) | `dev/requirements/requirements.md` | 0 | framework standard (layout / gates / kinds) |
...
same engine as these canon packs. `discoverPacks({ localRoot })` ([registry.mjs](../engine/packs/registry.mjs)) scans this repo's
...
- is **declared by hand** in `.claudinite-checks.json` like any pack — never fingerprinted or seeded
  ...
  the engine's [`packEntryId`](../engine/packs/registry.mjs) resolves it and the legacy bare id alike to the bare
...
This is **not a check** — a pack can't be imported without its dependencies, so the resolution happens **when the declaration is written**, at bootstrap `--init` and the baselining backfill ([bootstrap.md](../bootstrap.md) Part 6): [`resolveDeclaredPacks`](../engine/packs/registry.mjs) pulls each declared pack's transitive `requires` closure into `.claudinite-checks.json`. The prerequisite is materialized and visible in the file — droppable like every other entry, the same reason `basics` is written explicitly rather than defaulted — rather than resolved implicitly at run time. Declared ids keep their order; each pack's pulled-in dependencies land right after it.
...
[`env.mjs`](../engine/packs/env.mjs) drives everything from the repo's **active** packs (same activation as prose/checks):

- `node .claudinite/shared/engine/packs/env.mjs install` runs every active pack's `setup` in the checkout. The corpus's one generic [`environment-setup.sh`](../engine/mount/environment-setup.sh) (synced into every consumer's `.claudinite/mount/`) calls this after syncing the corpus.
- `node .claudinite/shared/engine/packs/env.mjs check` is a SessionStart hook (web only) that **asserts** — it runs each `probe` directly against the running environment and injects the halt-gate context if a requirement is missing. No version flag: the probes are the source of truth, and a genuinely new requirement fails its probe and prompts a re-run. Never installs.
- `node .claudinite/shared/engine/packs/env.mjs plan` prints what `install` would run (review / debug).
...
([interview.mjs](../engine/packs/interview.mjs)): at adoption every question is pending; when the canon later adds
