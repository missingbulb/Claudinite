...
| Notarize the distributed container, then staple it | high | legal | prose: 41 words + check (`notarytool-submit-waits`) |
...
| `notarytool-submit-waits` | high | legal | check: blocking |

Five checks, each on a rule whose static signature is false-positive-free *because the rule is
...
manifest that both state an OS floor, or a script that submits to the notary service. The rest stays prose: runtime device behaviour, a CI lane's
