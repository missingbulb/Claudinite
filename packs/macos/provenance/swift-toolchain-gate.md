## 2026-09-05 · born · converted from references.md (check:swift-toolchain-gate)
- **Reason:** `/usr/bin/swift` ships on every Mac as a stub: run without a developer directory it
  ...
  the toolchain-less Mac a diagnostic script is meant to degrade on, and is a test of the stub's
  presence rather than of a toolchain. Learned on LaughCounter, whose owner's Mac installs the DMG
  from CI and carries no Xcode tools.
- **Mechanism:** a check
