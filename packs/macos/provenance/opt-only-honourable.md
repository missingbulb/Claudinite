## 2026-08-12 · born · the rules distilled from a shipping Mac app (#756)
- **Source:** missingbulb/LaughCounter's `on-device-privacy` local pack, its `on-device-speech`
  check, re-read for what is portable macOS knowledge rather than that app's policy.
- **Actor:** @missingbulb (owner).
- **Mechanism:** a RULES.md rule, triggered on "The opt-in is only honourable where the locale's
  model is installed.".
- **Landed:** #756 (Closes #641) · pack version 1.
