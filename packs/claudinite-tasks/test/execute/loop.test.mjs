...
const task = (id, decl = {}, terms = RUNS) => ({
  ...
  decl: normalizeTaskDeclaration({ id, trigger: 'schedule', agent_model: 'sonnet', preconditions: ['gate'], expected_outcome: 'fresh_pr', ...decl }),
  terms,
});
...
test('dispatchWorkflow judges the POST by status, never by its body', async () => {
  const { dispatchWorkflow } = await import('../../src/world/github.mjs');
  const seen = [];
  const gh = async (path, opts) => {
    seen.push({ path, ...opts });
