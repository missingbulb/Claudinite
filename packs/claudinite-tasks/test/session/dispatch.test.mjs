...
test('a built-in dispatch validates from the pack path and the legacy engine path alike', async () => {
  const { validateDispatchBody } = await import('../../src/session/validate-dispatch.mjs');
  const { requestTaskPath } = await import('../../src/contract/built-in-tasks.mjs');
  ...
  const { loadTaskTerms } = await import('../../src/contract/task-terms.mjs');
