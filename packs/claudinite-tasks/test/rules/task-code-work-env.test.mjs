...
test('a task reading a variable code_work never sets is a finding', () => {
  const f = runRule(rule, ctx({ [WORKER]: 'const bag = process.env.CLAUDINITE_OVERRIDES;\n' }));
...
test('every variable the code_work contract sets is accepted', () => {
  ...
  assert.deepEqual(runRule(rule, ctx({ [WORKER]: src })), []);
});
...
test('a commented mention of a retired variable is not a read', () => {
  ...
  assert.deepEqual(runRule(rule, ctx({ [WORKER]: '// CLAUDINITE_OVERRIDES is gone — parameters ride Context\nprocess.env.CLAUDINITE_ITEM;\n' })), []);
});

test('only files under a task directory are in scope', () => {
  assert.deepEqual(runRule(rule, ctx({ 'engine/x.mjs': 'process.env.CLAUDINITE_OVERRIDES;\n' })), []);
});
...
test('a test beside a task is not the task', () => {
  ...
  assert.deepEqual(runRule(rule, ctx({ 'packs/demo/tasks/thing/worker.test.mjs': 'process.env.CLAUDINITE_OVERRIDES;\n' })), []);
});
...
test('the rule\'s scope is non-empty against this repo', () => {
  ...
  const seen = [];
  runRule(rule, { files, read: (f) => { const t = read(f); if (t !== null && /(^|\/)tasks\/[^/]+\/.+\.mjs$/.test(f) && !/\.test\.mjs$/.test(f)) seen.push(f); return t; } });
