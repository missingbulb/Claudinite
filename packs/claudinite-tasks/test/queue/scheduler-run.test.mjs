...
test('anchors are the slot schedule\'s instants with none of its identity', () => {
  ...
  // The retired spellings no longer resolve (#1234): the door passes them through and the
  // calendar throws rather than inventing an anchor for a token it does not know. Nothing can
  // reach here carrying one — `validateTaskDeclaration` rejects it at the door, and the fleet's
  // last such declaration moved to `daily` before this landed.
