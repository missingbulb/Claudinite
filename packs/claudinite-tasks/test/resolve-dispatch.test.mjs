...
const SHELL = join(dirname(fileURLToPath(import.meta.url)), '..', 'resolve-dispatch.mjs');
...
test('a consumer runs the VENDORED copy and resolves against its own root', () => {
  ...
  // copy at `.claudinite/shared/packs/claudinite-tasks/` — reached here through the
  // old engine/scheduler entry shim, the path a not-yet-repointed workflow names.
  ...
  try {
    ...
    cpSync(join(ENGINE, '..', 'packs', 'claudinite-tasks'),
      join(root, '.claudinite', 'shared', 'packs', 'claudinite-tasks'), { recursive: true });
