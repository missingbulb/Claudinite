...
async function deleteRef(gh, repo, ref, log) {
  if (!ref) return;
  const res = await deleteBranchRef(gh, repo, ref);
...
export async function resolveTarget({
  ...
  const listed = await listOpenPulls(gh, repo);
  ...
  for (const p of listed.json) {
    ...
    const res = await readCommit(gh, repo, p.head.sha);
  ...
  if (canonical === 'amend_existing_or_create_new_pr') {
    ...
    for (let n = 0; n < MERGEABLE_READS; n += 1) {
      ...
      const res = await readPull(gh, repo, newest.number);
  ...
  if (disposition === 'merge') {
    const res = await mergePull(gh, repo, newest.number,
      { merge_method: 'squash', commit_message: taskTrailer(taskId) });
    if (res.status === 200) {
...
export async function closeSuperseded({ gh, repo, numbers = [], successor, log = console.log }) {
  for (const number of numbers) {
    const read = await readPull(gh, repo, number);
    if (read.status !== 200) { log(`could not close #${number} (${read.status}) — leaving it`); continue; }
    await postComment(gh, repo, number,
      `Superseded by #${successor}, a later run of the same task. Closing this one.`);
    const closed = await closePull(gh, repo, number);
