...
//     (What `close` means is the CALLER's policy — a supersede resolution takes a
//     fresh branch and closes the previous delivery; a landing pass just leaves the
//     PR standing.)
//
...
export async function enableAutoMerge(token, pullRequestId) {
  ...
  const json = await graphqlCall(token, query, { id: pullRequestId });
...
export async function landNow({ token, repo, pr, delivery, expected = 0, task = null, log = console.log }) {
  ...
  for (;;) {
    ...
    const attempt = landAttempt({ delivery, runs, expected, elapsedMs: nowMs() - started });
