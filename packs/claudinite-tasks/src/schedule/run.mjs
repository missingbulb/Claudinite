...
import { actionsEnv, repoRoot, setStepOutput } from '../world/actions.mjs';
...
export async function listMarkedIssues(gh, repo, { permissionOf = null } = {}) {
  ...
  const push = permissionOf ?? (async (login) => {
    const res = await collaboratorPermission(gh, repo, login);
...
async function main() {
  ...
  for (const n of blockersToResolve(items, requests, known)) {
    const { status, json } = await getIssue(gh, repo, n);
