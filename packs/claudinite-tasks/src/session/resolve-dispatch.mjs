...
export function repoRootFrom(moduleUrl) {
  ...
  const home = dirname(dirname(dirname(dirname(dirname(fileURLToPath(moduleUrl))))));
...
export function readEventPayload(env = actionsEnv(), read = (p) => readFileSync(p, 'utf8')) {
...
export function triggerFromCcrEnv(env = actionsEnv()) {
...
export function resolveTrigger(env = actionsEnv(), read) {
  const reasons = [];
...
export async function resolveDispatch(argv = process.argv.slice(2), env = actionsEnv()) {
  ...
  if (DISPATCH_PATH_RE.test(firstLine) && existsSync(taskDir)) {
    ...
    try {
      const { loadTaskTerms } = await import('../contract/task-terms.mjs');
