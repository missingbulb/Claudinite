...
async function windowCommits(gh, repo, branch, sinceIso) {
  ...
  const detailed = [];
  for (const c of list) {
    const d = await readCommit(gh, repo, c.sha);
...
const COLLECTORS = {
  ...
  async prs(gh, ctx) {
    ...
    for (const p of [...inWindow, ...mergedCandidates]) {
      ...
      const head = await readCommit(gh, ctx.repo, sha);
  ...
  async branches(gh, ctx) {
    ...
    for (const sha of new Set(list.map((b) => b.commit?.sha).filter(Boolean))) {
      const { status, json } = await readCommit(gh, ctx.repo, sha);
  ...
  async release(gh, ctx) {
    const { status, json } = await latestRelease(gh, ctx.repo);
  ...
  async conversationLogs(gh, ctx) {
    ...
    const branch = await readBranch(gh, ctx.repo, 'conversation-logs');
    ...
    const { status, json } = await readTree(gh, ctx.repo, 'conversation-logs');
  ...
  async request(gh, ctx) {
    ...
    const { status, json } = await getIssue(gh, ctx.repo, number);
    ...
    const permissionOf = async (login) => {
      ...
      const res = await collaboratorPermission(gh, ctx.repo, login);
