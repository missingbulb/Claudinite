...
export function taskDeclarationFiles(root, activePacks) {
  ...
  const roots = activePacks.map((p) => ({ id: p.id, dir: join(p.dir, 'tasks') }));
  const builtIn = builtInTasksRoot(root);
  if (builtIn) roots.push({ id: BUILT_IN_PACK, dir: builtIn });

  const found = [];
  for (const pack of roots) {
    if (!existsSync(pack.dir)) continue;
    let names;
    try {
      names = readdirSync(pack.dir, { withFileTypes: true })
        .filter((d) => d.isDirectory()).map((d) => d.name).sort();
    } catch (e) {
      errors.push({ pack: pack.id, what: `${pack.id}'s tasks/ is not a readable directory: ${e.message}`, fix: `make ${relative(root, pack.dir)} a directory (or remove it)` });
      continue;
    }
...
export async function discoverTasks(root, config) {
  ...
  const { found, errors: walkErrors } = taskDeclarationFiles(root, active);
  ...
  for (const { pack, name, taskDir, file } of found) {
    ...
    if (problems.length) {
      errors.push({ pack, task: name, what: `${relative(root, file)} is not a valid task declaration: ${problems.map((p) => p.what).join('; ')}`, fix: problems[0].fix });
      continue;
    }
    ...
    if (decl.id !== name) {
      errors.push({ pack, task: name, what: `task in ${relative(root, taskDir)} declares id "${decl.id}" but its directory is "${name}"`, fix: 'rename the directory to the task id, or set the id to the directory name' });
      continue;
    }
    tasks.push({
      pack,
