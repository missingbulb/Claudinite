...
| `precondition-policy.mjs` | the precondition engine: the `preconditions` expression grammar (a conjunction, `\|\|` inside a term, inline `:` arguments, the run-history terms `due:`/`last-run-over:`/`last-run-not-failed`/`woken` that say when), the built-in term vocabulary and its three-valued semantics, the derived signal union, and the loud fail direction |
