...
async function main() {
  ...
  // The pack home first, then the pre-#1317 engine home (a member whose mount was
  // last written by an older engine still holds the stubs there), then the repo
  // root — the canon home runs this same code with no mount at all.
  const stubs = [
    join(root, '.claudinite/shared/packs/claudinite-tasks/stubs'),
    join(root, '.claudinite/shared/engine/scheduler/stubs'),
    join(root, 'packs/claudinite-tasks/stubs'),
  ].find((d) => existsSync(join(d, 'claudinite-scheduler.yml')));
  if (!stubs) { console.error(`converge-wiring: no vendored scheduler stub under ${join(root, '.claudinite/shared')}`); process.exit(1); }
