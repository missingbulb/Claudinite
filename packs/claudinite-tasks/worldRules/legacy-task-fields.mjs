...
// accepts the retired `frequency` field (which reads as the cadence term it always
// meant, docs/PRINCIPLES.md) and a declaration stating no `trigger` (whose value is
// derived from the shape of its conditions), so a task declared in the older
// vocabulary runs exactly like one declared today — and nothing told its author that
// the acceptance ends a convergence window after this advisory ships (#1732, #1789).
//
...
const rule = {
  ...
  severity: 'advisory',
  since: '2026-09-03',
  description: 'Task declarations name their cadence and trigger in the current vocabulary',
  why: 'the contract accepts the retired frequency field and an unstated trigger for one convergence window after this advisory ships (#1732, #1789) — nothing counts who is still on them, so a declaration not brought up to the vocabulary inside that window simply stops being read',
