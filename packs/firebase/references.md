...
- **(firestore-security-rules-1)** The create/merge asymmetry was recorded as the single most common way a
...
- **(firestore-security-rules-3)** Reading rules and believing them is how the merge-semantics bugs of RULES-1
...
- **(create-release-plan-4)** The failure is social, not technical: an unqualified `firebase deploy` copied
...
- **(firestore-security-rules-5)** The point of pinning to `request.time` is that a client must not be able to
  ...
  entirely.
- **(firebase-functions-6)** A build that emits an entrypoint which throws on `require` is invisible until
